...
public interface VectorStore extends DocumentWriter, VectorStoreRetriever {
	 ...
	 * <p>
	 * The default implementation fetches all document IDs and deletes them one by one.
	 * Subclasses should override this method to provide more efficient implementations
	 * using native bulk-delete APIs.
	 * <p>
	 * Note: This method may not be supported by all vector store implementations.
	 * In such cases, an {@link UnsupportedOperationException} may be thrown.
	 * @throws IllegalStateException if the underlying delete causes an exception
	 */
	default void deleteAll() {
		// Default implementation: fetch all IDs and delete them
		// Subclasses should override with efficient native implementations
		List<String> allIds = this.ids();
		if (!allIds.isEmpty()) {
			this.delete(allIds);
		}
	}
	...
	/**
	 * Returns all document IDs in the vector store.
	 * <p>
	 * This method is used by the default {@link #deleteAll()} implementation.
	 * Subclasses should override this method or {@link #deleteAll()} for efficiency.
	 * @return list of all document IDs
	 */
	default List<String> ids() {
		// Default implementation uses similarity search with high topK to retrieve all IDs
		// Subclasses should override with more efficient implementations
		List<Document> allDocs = this.similaritySearch(SearchRequest.builder().query("").topK(Integer.MAX_VALUE).build());
		return allDocs.stream().map(Document::getId).toList();
	}
