...
public class MockWeatherService implements Function<MockWeatherService.Request, MockWeatherService.Response> {
	...
	public record Request(
			@JsonProperty(required = true,
					value = "location") @JsonPropertyDescription("The city, example: San Francisco") Object location,
	...
	public Response apply(Request request) {
		...
		if (request.location().toString().contains("Paris")) {
			temperature = 15;
		}
		else if (request.location().toString().contains("Tokyo")) {
			temperature = 10;
		}
		else if (request.location().toString().contains("San Francisco")) {
			temperature = 30;
		}
