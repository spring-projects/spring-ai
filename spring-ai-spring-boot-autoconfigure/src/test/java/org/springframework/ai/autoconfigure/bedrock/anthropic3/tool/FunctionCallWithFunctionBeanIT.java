 ...
 */
package org.springframework.ai.autoconfigure.bedrock.anthropic3.tool;
...
import org.springframework.ai.autoconfigure.bedrock.anthropic3.BedrockAnthropic3ChatAutoConfiguration;
import org.springframework.ai.autoconfigure.bedrock.anthropic3.tool.MockWeatherService.Request;
import org.springframework.ai.autoconfigure.bedrock.anthropic3.tool.MockWeatherService.Response;
...
class FunctionCallWithFunctionBeanIT {
	...
	private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
		.withPropertyValues("spring.ai.bedrock.anthropic3.chat.enabled=true",
				...
				"spring.ai.bedrock.anthropic3.chat.model=" + Anthropic3ChatModel.CLAUDE_V3_SONNET.id(),
				"spring.ai.bedrock.anthropic3.chat.options.temperature=0.5")
		.withConfiguration(AutoConfigurations.of(SpringAiRetryAutoConfiguration.class,
				BedrockConverseApiAutoConfiguration.class, BedrockAnthropic3ChatAutoConfiguration.class))
	...
	void functionCallTest() {
		...
		contextRunner.run(context -> {
			...
			BedrockAnthropic3ChatModel chatModel = context.getBean(BedrockAnthropic3ChatModel.class);

			var userMessage = new UserMessage(
					"What's the weather like in San Francisco, Tokyo and Paris? Return the result in Celsius.");

			ChatResponse response = chatModel.call(new Prompt(List.of(userMessage),
					Anthropic3ChatOptions.builder().withFunction("weatherFunction").build()));
			...
			response = chatModel.call(new Prompt(List.of(userMessage),
					Anthropic3ChatOptions.builder().withFunction("weatherFunction3").build()));
