...
public class OpenAiChatClientToolFunction2IT {
	...
	void functionCallTest() {
		contextRunner
			...
			.run(context -> {
				...
				// handler.handleFunctionCall(messages, response1);
				ChatCompletion chatCompletion = (ChatCompletion) response1.getRawResponse();
				...
				if (responseMessage.toolCalls() != null) {
					...
					messages.add(new AssistantMessage(responseMessage.content(),
							Map.of("tool_calls", responseMessage.toolCalls())));
					...
					for (ToolCall toolCall : responseMessage.toolCalls()) {
						...
						messages.add(new ToolMessage(functionResponseContent, Map.of("tool_call_id", toolCall.id())));
					}
				}
