 ...
 * sharing the OpenAI error envelope (DeepSeek, Mistral AI, MiniMax, Ollama, Stability AI,
 * ElevenLabs, etc.) surface a status-prefixed message with an actionable hint instead of
 * a raw JSON blob.
 *
 * @author snowykte0426
 * @since 1.1.0
...
public final class ApiErrorMessageImprover {
	 ...
	 */
	public static String improveErrorMessage(final String rawError, final HttpStatusCode statusCode) {
	...
	private static String createImprovedMessage(final ApiErrorResponse.ErrorDetail errorDetail,
		...
		if (StringUtils.hasText(errorDetail.getMessage())) {
			...
			message.append("An error occurred with the AI provider API");
		}
	...
	private static String createFallbackMessage(final String rawError, final HttpStatusCode statusCode) {
		return String.format("%s - %s", statusCode.value(), rawError);
	}

	private static String getUserFriendlyPrefix(final HttpStatusCode statusCode) {
		...
		else if (HttpStatus.TOO_MANY_REQUESTS.isSameCodeAs(statusCode)) {
			return "Rate Limit Error: ";
		}
		else if (HttpStatus.INTERNAL_SERVER_ERROR.isSameCodeAs(statusCode)) {
			return "Server Error: ";
		}
		else if (HttpStatus.SERVICE_UNAVAILABLE.isSameCodeAs(statusCode)) {
	...
	private static @Nullable String getHelpfulHint(final @Nullable String errorCode) {
