...
class TranscriptionClientTests {
	...
	void transcrbeRequestReturnsResponseCorrectly() {
		...
		TranscriptionClient mockClient = Mockito.mock(TranscriptionClient.class);

		String mockTranscription = "All your bases are belong to us";

		// Create a mock Transcript
		Transcript transcript = Mockito.mock(Transcript.class);
		when(transcript.getOutput()).thenReturn(mockTranscription);

		// Create a mock TranscriptionResponse with the mock Transcript
		TranscriptionResponse response = Mockito.mock(TranscriptionResponse.class);
		...
		doAnswer(invocationOnMock -> {
			...
			TranscriptionRequest transcriptionRequest = invocationOnMock.getArgument(0);
