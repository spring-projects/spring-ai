...
/**
 * Unit tests for {@link VectorStoreAddRequestContentObservationFilter}.
 *
 * @author Christian Tzolov
 */
class VectorStoreAddRequestObservationFilterTests {
	...
	void whenNonEmptyQueryResponseThenAugmentContext() {
		...
		List<Document> addRequestContent = List.of(new Document("doc1"), new Document("doc2"));

		expectedContext.setAddRequest(addRequestContent);

		var augmentedContext = observationFilter.map(expectedContext);

		assertThat(augmentedContext.getHighCardinalityKeyValues())
			.contains(KeyValue.of(HighCardinalityKeyNames.ADD_REQUEST.asString(), "[\"doc1\", \"doc2\"]"));
	}
