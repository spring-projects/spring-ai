...
/**
 * Factory for creating {@link McpSyncClient} instances with consistent configuration.
 *
 * <p>
 * This factory encapsulates the common logic for building MCP synchronous clients,
 ...
 * {@code List<McpSyncClient>} bean) and {@link NamedMcpSyncClientFactoryBean} (for
 ...
 * @see McpSyncClientConfigurer
 */
public class McpSyncClientFactory {

	private final McpClientCommonProperties commonProperties;

	private final McpSyncClientConfigurer configurer;

	private final ClientMcpSyncHandlersRegistry handlersRegistry;

	public McpSyncClientFactory(McpClientCommonProperties commonProperties, McpSyncClientConfigurer configurer,
			ClientMcpSyncHandlersRegistry handlersRegistry) {
	...
	/**
	 * Creates an {@link McpSyncClient} for the given transport.
	 * @param namedTransport the named transport containing connection name and transport
	 * @return a fully configured and optionally initialized MCP sync client
	 */
	public McpSyncClient createClient(NamedClientMcpTransport namedTransport) {
		String connectionName = namedTransport.name();
		String clientName = this.commonProperties.getName() + " - " + connectionName;

		McpSchema.Implementation clientInfo = new McpSchema.Implementation(clientName, connectionName,
