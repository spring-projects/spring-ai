...
class TranscriptionObservationAutoConfigurationTests {
	...
	void meterObservationHandlerEnabled() {
		this.contextRunner.withBean(CompositeMeterRegistry.class)
			.run(context -> assertThat(context).hasSingleBean(TranscriptionModelMeterObservationHandler.class));
	}

	@Test
	void meterObservationHandlerDisabled() {
		this.contextRunner
			.run(context -> assertThat(context).doesNotHaveBean(TranscriptionModelMeterObservationHandler.class));
	}
