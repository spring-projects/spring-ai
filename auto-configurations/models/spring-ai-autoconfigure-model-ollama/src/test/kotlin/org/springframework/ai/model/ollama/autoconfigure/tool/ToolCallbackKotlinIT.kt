...
class ToolCallbackKotlinIT : BaseOllamaIT() {
	...
	fun toolCallTest() {
		this.contextRunner.run { context ->
			...
			val weatherTool = context.getBean(ToolCallback::class.java)

			val userMessage = UserMessage(
				"What are the weather conditions in San Francisco, Tokyo, and Paris? Find the temperature in Celsius for each of the three locations."
			)

			val functionOptions = OllamaChatOptions.builder().model(MODEL_NAME).toolCallbacks(weatherTool).build()
	...
	open class Config {
		...
		open fun weatherInfo(): ToolCallback {
			...
			return FunctionToolCallback
				.builder("weatherInfo", Function { request: KotlinRequest -> service.invoke(request) })
