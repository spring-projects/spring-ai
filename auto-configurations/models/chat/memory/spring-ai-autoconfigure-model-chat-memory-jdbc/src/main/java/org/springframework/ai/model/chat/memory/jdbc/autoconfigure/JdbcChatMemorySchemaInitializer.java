 ...
 * @since 1.0.0
 */
class JdbcChatMemorySchemaInitializer extends DataSourceScriptDatabaseInitializer {
	...
	JdbcChatMemorySchemaInitializer(DataSource dataSource, JdbcChatMemoryProperties properties) {
	...
	static DatabaseInitializationSettings getSettings(DataSource dataSource, JdbcChatMemoryProperties properties) {
		...
		String init = properties.getInitializeSchema();
		DatabaseInitializationMode mode;
		if ("always".equalsIgnoreCase(init)) {
			mode = DatabaseInitializationMode.ALWAYS;
		}
		else if ("never".equalsIgnoreCase(init)) {
