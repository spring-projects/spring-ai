...
class MongoChatMemoryAutoConfigurationIT {
	...
	void allMethodsShouldExecute() {
		...
		assertThat(chatMemoryRepository.findConversationIds().contains(conversationId)).isTrue();

		assertThat(chatMemoryRepository.findByConversationId(conversationId).size()).isEqualTo(1);

		chatMemoryRepository.deleteByConversationId(conversationId);

		assertThat(chatMemoryRepository.findByConversationId(conversationId).size()).isEqualTo(0);
	...
	void indicesShouldBeCreated() {
		...
		assertThat(mongoTemplate.indexOps(Conversation.class).getIndexInfo().size()).isEqualTo(2);
	}
