 ...
 * {@link GoogleGenAiEmbeddingConnectionAutoConfiguration}.
 *
 */
class GoogleGenAiEmbeddingConnectionModelConfigurationTests {

	private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
		.withConfiguration(AutoConfigurations.of(GoogleGenAiEmbeddingConnectionAutoConfiguration.class));

	@Test
	void connectionAvailableWhenTextEmbeddingModelEnabled() {
		this.contextRunner.withPropertyValues("spring.ai.google.genai.embedding.api-key=test-key").run(context -> {
	...
	@Test
	void connectionBacksOffWhenTextEmbeddingModelSetToNone() {
		this.contextRunner
	...
	@Test
	void connectionBacksOffWhenAnotherTextEmbeddingModelIsSelected() {
		this.contextRunner
			.withPropertyValues("spring.ai.model.embedding.text=openai",
					"spring.ai.google.genai.embedding.api-key=test-key")
			.run(context -> {
				assertThat(context).doesNotHaveBean(GoogleGenAiEmbeddingConnectionDetails.class);
				assertThat(context).hasNotFailed();
