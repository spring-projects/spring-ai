 ...
 * </p>
 * <ul>
 * <li>{@link BedrockAgentRuntimeClient} - using Spring Cloud AWS credentials if available,
 * otherwise falling back to the default AWS credentials chain</li>
 * <li>{@link BedrockKnowledgeBaseVectorStore} - configured from properties</li>
 * </ul>
 *
 * <p>
 * The {@link BedrockAgentRuntimeClient} bean will use an existing
 * {@link AwsCredentialsProvider} bean if available (e.g., from Spring Cloud AWS), ensuring
 * unified credential management across all Bedrock integrations in the application.
 * </p>
 *
 * <p>
...
public class BedrockKnowledgeBaseVectorStoreAutoConfiguration {
	...
	/**
	 * Creates a BedrockAgentRuntimeClient using Spring Cloud AWS credentials if available,
	 * otherwise falling back to the default AWS credentials chain. This bean is only created
	 * if no other BedrockAgentRuntimeClient is defined.
	 * @param credentialsProvider the AWS credentials provider (may be null if no provider is
	 * configured)
