 ...
 * <a href="https://jspecify.dev/docs/user-guide/">JSpecify annotations</a> are fully
 * supported, as well as <a href="https://kotlinlang.org/docs/null-safety.html">Kotlin
 * null safety</a>, {@code @Nullable} annotations regardless of their package, and Java
 * primitive types.
 *
 * <p>
 ...
 * {@code @NonNullFields}, and {@code @NonNull} are not supported by this API. However,
 * {@code @Nullable} is supported via the package-less check. Migrating to JSpecify is
 * recommended.
 *
...
public enum Nullness {
	...
	public static Nullness forParameter(Parameter parameter) {
		if (KOTLIN_REFLECT_PRESENT
				&& KotlinDetector.isKotlinType(parameter.getDeclaringExecutable().getDeclaringClass())) {
			// TODO Optimize when kotlin-reflect provide a more direct Parameter to
			// KParameter resolution
	...
	private static Nullness jSpecifyNullness(AnnotatedElement annotatedElement, Class<?> declaringClass,
				...
				if (methodName.startsWith("get")) {
					...
					for (KProperty<?> property : KClasses.getMemberProperties(kClass)) {
						if (property.getName().equals(propertyName)) {
							return (property.getReturnType().isMarkedNullable() ? Nullness.NULLABLE
									: Nullness.NON_NULL);
						}
					}
				}
