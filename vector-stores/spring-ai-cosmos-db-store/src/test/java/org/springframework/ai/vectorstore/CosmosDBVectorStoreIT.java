...
public class CosmosDBVectorStoreIT {
	...
	private void emulatorSetup(CosmosDBEmulatorContainer cosmosDBEmulatorContainer){
		...
		Path keyStoreFile = tempFolder.newFile("v1-azure-cosmos-emulator.keystore").toPath();
		...
		tempFolder.delete(keyStoreFile.toFile());
	}
	...
	public static class TestApplication {
		...
		public VectorStore vectorStore(CosmosAsyncClient cosmosClient, EmbeddingModel embeddingModel, VectorStoreObservationConvention convention) {
			cosmosClient.createDatabaseIfNotExists("test-database").block();
			return new CosmosDBVectorStore(null, convention,  cosmosClient,
					"test-database", "test-container", embeddingModel);
		}
