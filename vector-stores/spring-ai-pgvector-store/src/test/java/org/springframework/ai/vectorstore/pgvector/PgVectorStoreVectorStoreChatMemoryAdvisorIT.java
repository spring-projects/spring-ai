...
public class PgVectorStoreVectorStoreChatMemoryAdvisorIT {
	...
	void testSemanticSearchRetrievesRelevantMemory() throws Exception {
		...
		ChatClient chatClient = ChatClient.builder(chatModel)
			.defaultAdvisors(VectorStoreChatMemoryAdvisor.builder(store).defaultChatMemoryRetrieveSize(1).build())
			.build();
	...
	void testIrrelevantMessageExclusion() throws Exception {
		...
		ChatClient chatClient = ChatClient.builder(chatModel)
			.defaultAdvisors(VectorStoreChatMemoryAdvisor.builder(store).defaultChatMemoryRetrieveSize(2).build())
			.build();
