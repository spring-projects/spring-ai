...
class S3FilterExpressionConverterTests {
	...
	public void testDate() {
		Document vectorExpr = this.converter.convertExpression(new Filter.Expression(EQ, new Filter.Key("activationDate"),
				new Filter.Value(new Date(1704637752148L))));
		Document filter = Document.fromMap(Map.of(
				"activationDate", Document.fromMap(Map.of(
						"$eq", Document.fromString("2024-01-07T14:29:12Z")
				))
		));
		...
		filter = Document.fromMap(Map.of(
				"activationDate", Document.fromMap(Map.of(
						"$eq", Document.fromString("1970-01-01T00:00:02Z")
				))
		));
		assertThat(vectorExpr).isEqualTo(filter);
	}
	...
	@Test
	public void testEQ() {
		Document vectorExpr = this.converter
				.convertExpression(new Filter.Expression(EQ, new Filter.Key("country"), new Filter.Value("BG")));

		Document filter = Document.fromMap(Map.of(
				"country", Document.fromMap(Map.of(
						"$eq", Document.fromString("BG")
				))
		));
	...
	public void tesEqAndGte() {
		Document vectorExpr = this.converter.convertExpression(new Filter.Expression(AND,
		...
		Document filter = Document.fromMap(Map.of(
				"$and", Document.fromList(List.of(
						Document.fromMap(Map.of(
								"genre", Document.fromMap(Map.of(
										"$eq", Document.fromString("drama")
								))
						)),
						Document.fromMap(Map.of(
								"year", Document.fromMap(Map.of(
										"$gte", Document.fromNumber(2020)
								))
						))
				))
		));
	...
	public void tesIn() {
		...
		Document vectorExpr = this.converter.convertExpression(new Filter.Expression(IN, new Filter.Key("genre"),
				new Filter.Value(genres)));


		Document filter = Document.fromMap(Map.of(
				"genre", Document.fromMap(Map.of(
						"$in", Document.fromList(
								genres.stream()
	...
	public void testGroup() {
		...
		Document filter = Document.fromMap(Map.of(
				"$and", Document.fromList(List.of(
						...
						Document.fromMap(Map.of(
								"city", Document.fromMap(Map.of(
										"$nin", Document.fromList(List.of(
												Document.fromString("Sofia"),
		...
		Document filter = Document.fromMap(Map.of(
				"$and", Document.fromList(List.of(
						Document.fromMap(Map.of(
								"$and", Document.fromList(List.of(
										Document.fromMap(Map.of(
												"isOpen", Document.fromMap(Map.of(
														"$eq", Document.fromBoolean(true)
												))
										)),
										Document.fromMap(Map.of(
												"year", Document.fromMap(Map.of(
	...
	public void testDecimal() {
		...
		Document filter = Document.fromMap(Map.of(
				"$and", Document.fromList(List.of(
						Document.fromMap(Map.of(
								"temperature", Document.fromMap(Map.of(
										"$gte", Document.fromNumber(-15.6)
								))
						)),
						Document.fromMap(Map.of(
								"temperature", Document.fromMap(Map.of(
										"$lte", Document.fromNumber(20.13)
								))
						))
				))
		));
	...
	public void testComplexIdentifiers() {
		...
		Document filter = Document.fromMap(Map.of(
				"\"country 1 2 3\"", Document.fromMap(Map.of(
						"$eq", Document.fromString("BG")
				))
		));
