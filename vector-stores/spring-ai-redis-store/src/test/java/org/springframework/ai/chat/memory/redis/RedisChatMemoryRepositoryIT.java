...
/**
 * Integration tests for RedisChatMemory implementation of ChatMemoryRepository interface.
 *
...
class RedisChatMemoryRepositoryIT {
	...
	void shouldEfficientlyFindAllConversationIdsWithAggregation() {
		this.contextRunner.run(context -> {
			...
			// Just log the performance - we don't assert on it as it might vary by
			// environment
			logger.info("findConversationIds took {} ms for 30 messages across 3 conversations", endTime - startTime);

			// The real verification that Redis aggregation is working is handled by the
			// debug logs in RedisChatMemory.findConversationIds
		});
	}
