...
import org.springframework.ai.wordlift.api.WordLiftApi.ChatCompletionChunk.ChunkChoice;
import org.springframework.ai.wordlift.api.WordLiftApi.ChatCompletionMessage.ChatCompletionFunction;
import org.springframework.ai.wordlift.api.WordLiftApi.ChatCompletionMessage.Role;
 ...
 * <p>
 * This is a copy of the original `OpenAIStreamFunctionCallingHelper` class by Christian Tzolov.
 *
 * @author David Riccitelli
 */
public class WordLiftStreamFunctionCallingHelper {
   ...
   */
  public ChatCompletionChunk merge(
    ChatCompletionChunk previous,
    ...
    Long created =
      (current.created() != null ? current.created() : previous.created());
    String model =
      (current.model() != null ? current.model() : previous.model());
    String systemFingerprint =
      (
        current.systemFingerprint() != null
          ? current.systemFingerprint()
          : previous.systemFingerprint()
      );
    String object =
      (current.object() != null ? current.object() : previous.object());
    Usage usage =
      (current.usage() != null ? current.usage() : previous.usage());

    ChunkChoice previousChoice0 =
      (
        CollectionUtils.isEmpty(previous.choices())
          ? null
          : previous.choices().get(0)
      );
    ChunkChoice currentChoice0 =
      (
        CollectionUtils.isEmpty(current.choices())
          ? null
          : current.choices().get(0)
      );

    ChunkChoice choice = merge(previousChoice0, currentChoice0);
    List<ChunkChoice> chunkChoices = choice == null
      ? List.of()
      : List.of(choice);
  ...
  private ChunkChoice merge(ChunkChoice previous, ChunkChoice current) {
    ...
    Integer index =
      (current.index() != null ? current.index() : previous.index());
  ...
  private ChatCompletionMessage merge(
    ChatCompletionMessage previous,
    ChatCompletionMessage current
  ) {
    String content =
      (
    ...
    if (previous.toolCalls() != null) {
      lastPreviousTooCall =
        previous.toolCalls().get(previous.toolCalls().size() - 1);
      if (previous.toolCalls().size() > 1) {
        toolCalls.addAll(
          previous.toolCalls().subList(0, previous.toolCalls().size() - 1)
        );
      }
    }
  ...
  private ToolCall merge(ToolCall previous, ToolCall current) {
    ...
    ChatCompletionFunction function = merge(
      previous.function(),
      current.function()
    );
  ...
  private ChatCompletionFunction merge(
    ChatCompletionFunction previous,
   ...
   */
  public boolean isStreamingToolFunctionCall(
    ChatCompletionChunk chatCompletion
  ) {
    if (
      chatCompletion == null ||
      CollectionUtils.isEmpty(chatCompletion.choices())
    ) {
      return false;
    }
   ...
   * the last one.
   */
  public boolean isStreamingToolFunctionCallFinish(
    ChatCompletionChunk chatCompletion
  ) {
    if (
      chatCompletion == null ||
