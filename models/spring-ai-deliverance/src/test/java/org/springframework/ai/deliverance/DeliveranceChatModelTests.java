...
class DeliveranceChatModelTests {
	...
	void mapsPromptAndOptionsToRequest() {
		...
		assertThat(request.getTemperature().doubleValue()).isEqualTo(1.0);
		assertThat(request.getMaxTokens()).isEqualTo(64);
		assertThat(request.getTopP().doubleValue()).isEqualTo(0.95);
		assertThat(request.getUniformTopP().doubleValue()).isEqualTo(1.0);
	...
	void callMapsResponseMetadataAndFinishReason() {
		...
		assertThat(((Number) response.getMetadata().get("created")).intValue()).isEqualTo(42);
		assertThat(response.getResult().getMetadata().getFinishReason()).isEqualTo("stop");
	}

	@Test
	void skipsToolCallsWithoutFunction() {
		ChatCompletionMessageToolCall toolCall = new ChatCompletionMessageToolCall().id("call_1").type("function");

		List<AssistantMessage.ToolCall> toolCalls = DeliveranceApi.toolCalls(List.of(toolCall));
	...
	void apiCanListModels() {
		...
		assertThat(response.getData().get(0).getId()).isEqualTo("test-model");
	}
	...
	void serializesRequestWithSpringGeneratedMapper() throws Exception {
		...
		assertThat(json).contains("\"top_p\":0.95");
		assertThat(json).contains("\"uniform_top_p\":1.0");
	...
	private static final class NoopDeliveranceApi implements DeliveranceApi {
		...
		public ListModelsResponse listModels() {
			return new ListModelsResponse()
				.data(List.of(new Model().id("test-model").created(42).ownedBy("deliverance")));
		}
