...
public class DeepSeekApiIT {
	...
	void chatCompletionEntity() {
		...
		ResponseEntity<ChatCompletion> response = this.deepSeekApi.chatCompletionEntity(
				new ChatCompletionRequest(List.of(chatCompletionMessage), ChatModel.DEEPSEEK_FLASH.value, 1D, false));
	...
	void chatCompletionStream() {
		...
		Flux<ChatCompletionChunk> response = this.deepSeekApi.chatCompletionStream(
				new ChatCompletionRequest(List.of(chatCompletionMessage), ChatModel.DEEPSEEK_FLASH.value, 1D, true));
