...
final class ResponsesRequestBuilder {
	...
	static ResponseCreateParams build(Prompt prompt, ToolCallingManager toolCallingManager) {
		...
		if (options.getTopLogprobs() != null) {
			builder.topLogprobs(options.getTopLogprobs().longValue());
		}
	...
	private static @Nullable Reasoning reasoning(OpenAiChatOptions options) {
	 ...
	 * level.
	 */
	private static @Nullable ResponseTextConfig text(OpenAiChatOptions options) {
	...
	private static List<Tool> tools(OpenAiChatOptions options, ToolCallingManager toolCallingManager) {
		...
		if (options.getServersideTools() != null) {
			options.getServersideTools().forEach(serversideTool -> tools.add(serversideTool.toTool()));
		}
		return tools;
	}
	 ...
	 * a function or a server-side tool type.
	 */
			...
			default -> {
				try {
					...
					if ("function".equals(type)) {
						...
						// A server-side tool type, e.g. {"type":"web_search"}
