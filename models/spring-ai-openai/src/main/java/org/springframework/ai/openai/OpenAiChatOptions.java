...
public class OpenAiChatOptions implements ToolCallingChatOptions {
   ...
   * from -100 to 100. Mathematically, the bias is added to the logits generated by the model prior
   * to sampling. The exact effect will vary per model, but values between -1 and 1 should decrease
   ...
   *
   * <p><strong>Model-specific usage:</strong>
   *
   * <ul>
   *   <li><strong>Use for non-reasoning models</strong> (e.g., gpt-4o, gpt-3.5-turbo)
   *   <li><strong>Cannot be used with reasoning models</strong> (e.g., o1, o3, o4-mini series)
   * </ul>
   *
   * <p><strong>Mutual exclusivity:</strong> This parameter cannot be used together with {@link
   * #maxCompletionTokens}. Setting both will result in an API error.
   */
   ...
   *
   * <ul>
   *   <li><strong>Required for reasoning models</strong> (e.g., o1, o3, o4-mini series)
   *   <li><strong>Cannot be used with non-reasoning models</strong> (e.g., gpt-4o, gpt-3.5-turbo)
   * </ul>
   *
   * <p><strong>Mutual exclusivity:</strong> This parameter cannot be used together with {@link
   * #maxTokens}. Setting both will result in an API error.
   */
   ...
   * capable of generating text, which is the default. The gpt-4o-audio-preview model can also be
   * used to generate audio. To request that this model generate both text and audio responses, you
   * can use: ["text", "audio"]. Note that the audio modality is only available for the
   * gpt-4o-audio-preview model and is not supported for streaming completions.
   */
   ...
   * modalities: ["audio"] Note: that the audio modality is only available for the
   * gpt-4o-audio-preview model and is not supported for streaming completions.
   */
  ...
  /**
   * Options for streaming response. Included in the API only if streaming-mode completion is
   * requested.
   */
   ...
   * the same result. Determinism is not guaranteed, and you should refer to the system_fingerprint
   ...
   * default when no functions are present. auto is the default if functions are present. Use the
   * {@link ToolChoiceBuilder} to create a tool choice object.
   */
  ...
  /**
   * Whether to enable <a
   * href="https://platform.openai.com/docs/guides/function-calling/parallel-function-calling">parallel
   * function calling</a> during tool use. Defaults to true.
   */
  private @JsonProperty("parallel_tool_calls") Boolean parallelToolCalls;

  /**
   * Whether to store the output of this chat completion request for use in our model <a
   * href="https://platform.openai.com/docs/guides/distillation">distillation</a> or <a
   * href="https://platform.openai.com/docs/guides/evals">evals</a> products.
   */
  private @JsonProperty("store") Boolean store;

  /**
   * Developer-defined tags and values used for filtering completions in the <a
   * href="https://platform.openai.com/chat-completions">dashboard</a>.
   */
   ...
   * used on reasoning in a response. Optional. Defaults to medium. Only for 'o1' models.
   */
  private @JsonProperty("reasoning_effort") String reasoningEffort;

  /**
   * verbosity: string or null Optional - Defaults to medium Constrains the verbosity of the model's
   * response. Lower values will result in more concise responses, while higher values will result
   * in more verbose responses. Currently supported values are low, medium, and high. If specified,
   * the model will use web search to find relevant information to answer the user's question.
   */
  ...
  /**
   * Specifies the <a
   * href="https://platform.openai.com/docs/api-reference/responses/create#responses_create-service_tier">processing
   * type</a> used for serving the request.
   */
  ...
  @JsonIgnore private Map<String, String> httpHeaders = new HashMap<>();
