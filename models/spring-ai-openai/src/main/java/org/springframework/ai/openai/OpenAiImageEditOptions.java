...
public class OpenAiImageEditOptions implements ImageEditOptions {

	/**
	 * An additional image whose fully transparent areas (e.g. where alpha is zero)
	 * indicate where image should be edited. If there are multiple images provided, the
	 * mask will be applied on the first image. Must be a valid PNG file, less than 4MB,
	...
	public boolean equals(Object o) {
		...
		if (!(o instanceof OpenAiImageEditOptions that)) {
			return false;
		}
		return Objects.equals(this.mask, that.mask) && Objects.equals(this.n, that.n)
				&& Objects.equals(this.model, that.model) && Objects.equals(this.width, that.width)
	...
	@Override
	public int hashCode() {
		return Objects.hash(this.mask, this.n, this.model, this.width, this.height, this.quality, this.responseFormat,
				this.size, this.user);
	}

	@Override
	public String toString() {
		return "OpenAiImageEditOptions{" + "mask=" + this.mask + "n=" + this.n + ", model='" + this.model + '\''
				+ ", width=" + this.width + ", height=" + this.height + ", quality='" + this.quality + '\''
				+ ", responseFormat='" + this.responseFormat + '\'' + ", size='" + this.size + '\'' + ", style='" + '\''
				+ ", user='" + this.user + '\'' + '}';
	}

	public static final class Builder {

		private final OpenAiImageEditOptions options;

		private Builder() {
			this.options = new OpenAiImageEditOptions();
		}
		...
		public OpenAiImageEditOptions build() {
			return this.options;
		}
