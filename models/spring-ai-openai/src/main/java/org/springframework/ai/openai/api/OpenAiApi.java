...
public class OpenAiApi {
	...
	/**
	 * Represents a tool the generative may call. Currently, only functions are supported as a tool.
	 *
	...
	public record FunctionTool(
		 ...
		 * Function definition.
		 *
		 * @param description A description of what the function does, used by the generative to choose when and how to call
		 * the function.
	...
	/**
	 * Creates a generative response for the given chat conversation.
	 *
	 * @param messages A list of messages comprising the conversation so far.
	 * @param model ID of the generative to use.
	 * @param frequencyPenalty Number between -2.0 and 2.0. Positive values penalize new tokens based on their existing
	 * frequency in the text so far, decreasing the generative's likelihood to repeat the same line verbatim.
	 ...
	 * Mathematically, the bias is added to the logits generated by the generative prior to sampling. The exact effect will
	 * vary per generative, but values between -1 and 1 should decrease or increase likelihood of selection; values like -100
	 * or 100 should result in a ban or exclusive selection of the relevant token.
	 * @param maxTokens The maximum number of tokens to generate in the chat completion. The total length of input
	 * tokens and generated tokens is limited by the generative's context length.
	 ...
	 * appear in the text so far, increasing the generative's likelihood to talk about new topics.
	 * @param responseFormat An object specifying the format that the generative must output. Setting to { "type":
	 * "json_object" } enables JSON mode, which guarantees the message the generative generates is valid JSON.
	 ...
	 * @param topP An alternative to sampling with temperature, called nucleus sampling, where the generative considers the
	 * results of the tokens with top_p probability mass. So 0.1 means only the tokens comprising the top 10%
	 * probability mass are considered. We generally recommend altering this or temperature but not both.
	 * @param tools A list of tools the generative may call. Currently, only functions are supported as a tool. Use this to
	 * provide a list of functions the generative may generate JSON inputs for.
	 * @param toolChoice Controls which (if any) function is called by the generative. none means the generative will not call a
	 * function and instead generates a message. auto means the generative can pick between generating a message or calling a
	 * function. Specifying a particular function via {"type: "function", "function": {"name": "my_function"}} forces
	 * the generative to call that function. none is the default when no functions are present. auto is the default if
	...
	public record ChatCompletionRequest(
		...
		/**
		 * Shortcut constructor for a chat completion request with the given messages and generative.
		 *
		 * @param messages A list of messages comprising the conversation so far.
		 * @param model ID of the generative to use.
		...
		/**
		 * Shortcut constructor for a chat completion request with the given messages, generative and control for streaming.
		 *
		 * @param messages A list of messages comprising the conversation so far.
		 * @param model ID of the generative to use.
		 * @param temperature What sampling temperature to use, between 0 and 1.
		...
		/**
		 * Shortcut constructor for a chat completion request with the given messages, generative, tools and tool choice.
		 * Streaming is set to false, temperature to 0.8 and all other parameters are null.
		 *
		 * @param messages A list of messages comprising the conversation so far.
		 * @param model ID of the generative to use.
		 * @param tools A list of tools the generative may call. Currently, only functions are supported as a tool.
		 * @param toolChoice Controls which (if any) function is called by the generative.
		 */
		...
		/**
		 * Specifies a tool the generative should use. Use to force the generative to call a specific function.
		 *
		 * @param type The type of the tool. Currently, only 'function' is supported.
		...
		/**
		 * An object specifying the format that the generative must output.
	 ...
	 * @param name An optional name for the participant. Provides the generative information to differentiate between
	 * participants of the same role.
	 * @param toolCallId Tool call that this message is responding to. Only applicable for the {@link Role#tool} role
	 * and null otherwise.
	 * @param toolCalls The tool calls generated by the generative, such as function calls. Applicable only for
	 ...
	 * called, as generated by the generative.
	 */
	@JsonInclude(Include.NON_NULL)
	public record ChatCompletionMessage(
		 ...
		 * @param arguments The arguments that the generative expects you to pass to the function.
		 */
		@JsonInclude(Include.NON_NULL)
	...
	/**
	 * The reason the generative stopped generating tokens.
	 */
	public enum ChatCompletionFinishReason {
		/**
		 * The generative hit a natural stop point or a provided stop sequence.
		 */
		stop,
		/**
		...
		content_filter,
		/**
		 * The generative called a tool.
		 */
		tool_calls,
		/**
		 * (deprecated) The generative called a function.
		 */
		function_call
	}

	/**
	 * Represents a chat completion response returned by generative, based on the provided input.
	 *
	 ...
	 * @param model The generative used for the chat completion.
	 * @param systemFingerprint This fingerprint represents the backend configuration that the generative runs with. Can be
	...
	public record ChatCompletion(
		 ...
		 *
		 * @param finishReason The reason the generative stopped generating tokens.
		 * @param index The index of the choice in the list of choices.
		 * @param message A chat completion message generated by the generative.
	...
	/**
	 * Represents a streamed chunk of a chat completion response returned by generative, based on the provided input.
	 *
	...
	public record ChatCompletionChunk(
		 ...
		 * @param finishReason The reason the generative stopped generating tokens.
		 * @param index The index of the choice in the list of choices.
		 * @param delta A chat completion delta generated by streamed generative responses.
		 * @param logprobs Log probability information for the choice.
		 */
		@JsonInclude(Include.NON_NULL)
	 ...
	 * @param embedding The embedding vector, which is a list of floats. The length of vector depends on the generative.
	 ...
	 * the generative (8192 tokens for text-embedding-ada-002), cannot be an empty string, and any array must be 2048
	 * dimensions or less.
	 * @param model ID of the generative to use.
	...
	public record EmbeddingRequest<T>(
		...
		/**
		 * Create an embedding request with the given input, generative and encoding format set to float.
		 ...
		 * generative is set to 'text-embedding-ada-002'.
		 * @param input Input text to embed.
		 */
	...
	public <T> ResponseEntity<EmbeddingList<Embedding>> embeddings(EmbeddingRequest<T> embeddingRequest) {
		...
		// The input must not exceed the max input tokens for the generative (8192 tokens for text-embedding-ada-002), cannot
