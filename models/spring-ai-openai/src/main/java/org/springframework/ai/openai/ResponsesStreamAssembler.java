 ...
 * nothing streamed for - a function call, a server-side tool item, generated media - is
...
final class ResponsesStreamAssembler {
	...
	List<ChatResponse> apply(ResponseStreamEvent event) {
		...
		// Everything else - lifecycle and server-side tool progress events - is already
	 ...
	 * re-serialize every server-side tool item a second time.
	 */
