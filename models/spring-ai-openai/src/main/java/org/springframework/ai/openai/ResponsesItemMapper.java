...
final class ResponsesItemMapper {
	...
	static ChatGenerationMetadata toGenerationMetadata(Response response, List<ResponseOutputItem> outputItems,
		...
		ChatGenerationMetadata.Builder metadata = ChatGenerationMetadata.builder()
			...
			.metadata(OpenAiResponsesMetadata.SERVERSIDE_TOOL_CALLS, reported.serversideToolCalls());
	...
	private static ReportedValues reportedValues(List<ResponseOutputItem> outputItems) {
		...
		List<Map<String, Object>> serversideToolCalls = new ArrayList<>();
		...
		for (ResponseOutputItem item : outputItems) {
			...
			if (message.isPresent()) {
				...
				serversideToolCalls.add(summarize(item));
			}
		}
		return new ReportedValues(refusal.toString(), reasoning.toString(), annotations, serversideToolCalls,
				hasToolCalls);
	}
	...
	static MessagePart toPart(ResponseOutputItem item) {
		...
		return serversideToolPart(item);
	}
	 ...
	 * <p>
	 * Image generation is the one server-side tool whose result the application wants
	 * back, so a call that produced an image becomes a {@link MediaPart} instead. That
	 * part is not replayed: a generated image is output rather than context.
	 */
	static MessagePart serversideToolPart(ResponseOutputItem item) {
	 ...
	 * @param serversideToolCalls a {@code {type, id, status}} map per server-side tool
	...
	private record ReportedValues(String refusal, String reasoning, List<Map<String, Object>> annotations,
			List<Map<String, Object>> serversideToolCalls, boolean hasToolCalls) {
	}
