 ...
 * response.
 * <p>
 * Hosted tools are not {@link org.springframework.ai.tool.ToolCallback ToolCallbacks}:
 * there is nothing for the application to run, and their calls are never surfaced as
 * {@link org.springframework.ai.chat.messages.AssistantMessage.ToolCall tool calls} -
 * they appear in the turn transcript and, summarized, under
 * {@link OpenAiResponsesMetadata#HOSTED_TOOL_CALLS} on the generation metadata.
 * <p>
...
public sealed interface HostedTool {
	 ...
	 * restriction
	 */
	record WebSearch(@Nullable String searchContextSize, @Nullable List<String> allowedDomains) implements HostedTool {
	 ...
	 * default
	 */
	record FileSearch(List<String> vectorStoreIds, @Nullable Integer maxNumResults) implements HostedTool {
	 ...
	 * one per request
	 */
	record CodeInterpreter(@Nullable String containerId) implements HostedTool {
	 ...
	 * @param requireApproval {@code never}, or {@code null} for the provider default.
	 * {@code always} is rejected: the approval round-trip is not implemented, so it would
	 * end every tool-using turn without running the tool
	 */
	record Mcp(String serverLabel, @Nullable String serverUrl, @Nullable String connectorId,
			...
			@Nullable String requireApproval) implements HostedTool {

		public Mcp {
			Assert.hasText(serverLabel, "serverLabel must not be empty");
			Assert.isTrue(!"always".equals(requireApproval),
					"requireApproval=always is not supported: the MCP approval round-trip is not implemented, "
							+ "so an mcp_approval_request would end the turn without running the tool. Use never.");
		}
	 ...
	 */
	record ImageGeneration(@Nullable String model, @Nullable String size) implements HostedTool {
	...
	record Raw(Map<String, Object> tool) implements HostedTool {
		...
		public Tool toTool() {
			...
			Assert.state(converted != null, () -> "Could not convert hosted tool declaration: " + this.tool);
			return converted;
		}
