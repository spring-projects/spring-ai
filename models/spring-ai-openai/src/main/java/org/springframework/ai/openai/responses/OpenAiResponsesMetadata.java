...
/**
 * Metadata keys published by {@link org.springframework.ai.openai.OpenAiChatModel
 * OpenAiChatModel} when it talks to the Responses API, and the part attributes it uses.
 * <p>
 ...
 * {@link #ANNOTATIONS} and {@link #REASONING_CONTENT} keep the unprefixed names the Chat
 * Completions endpoint publishes on its assistant message, so code that switches endpoint
 * only has to change where it reads them, not what it looks up.
 *
...
public final class OpenAiResponsesMetadata {
	 ...
	 * {@link org.springframework.ai.chat.messages.part.UnknownPart} this endpoint
	 * produces, and the only one it replays.
	 * <p>
	 ...
	 */
	public static final String SERVERSIDE_TOOL_CALLS = "openai.responses.serverside_tool_calls";
