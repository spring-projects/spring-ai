...
/**
 * Integration tests for {@link OpenAiChatModel} against the Responses API.
 *
 * @author Dimitar Proynov
 */
@SpringBootTest(classes = OpenAiChatModelResponsesApiIT.Config.class)
@EnabledIfEnvironmentVariable(named = "OPENAI_API_KEY", matches = ".+")
class OpenAiChatModelResponsesApiIT {
	...
	/**
	 * The endpoint is pinned rather than left to {@link Api#AUTO}, both on the bean below
	 * and on every request that carries native options, because those replace the model's
	 * defaults wholesale - the endpoint selection included.
	 */
	private static OpenAiChatOptions.Builder responsesOptions() {
		return OpenAiChatOptions.builder().api(Api.RESPONSES);
	}
	...
	void reasoningIsSurfacedAsAReplayablePart() {
		var options = responsesOptions().model("gpt-5.6-luna").reasoningEffort("high").reasoningSummary("auto").build();
		...
		assertThat(message.getReasoning())
			.as("transcript %s, server-side tool calls %s, reasoningContent '%s'", message.getParts(),
					metadata.get(OpenAiResponsesMetadata.SERVERSIDE_TOOL_CALLS),
	...
	void maxOutputTokensTruncatesTheAnswer() {
		var options = responsesOptions().model("gpt-5.6-luna").maxOutputTokens(16).build();
	...
	void reasoningWithToolCallingThroughTheAdvisor() {
		var options = responsesOptions().model("gpt-5.6-luna").reasoningEffort("low").reasoningSummary("auto").build();
	...
	@Test
	void serversideWebSearchDoesNotLookLikeALocalToolCall() {
		var options = responsesOptions().model("gpt-5.6-luna").serversideTools(ServersideTool.WebSearch.of()).build();
		...
		// The server-side call surfaces as metadata only, and the citations it
		// produced are the reason to enable the tool in the first place.
		assertThat(metadata.<List<Map<String, Object>>>get(OpenAiResponsesMetadata.SERVERSIDE_TOOL_CALLS))
			.as("transcript %s", message.getParts())
			.isNotEmpty()
			.allSatisfy(serversideCall -> assertThat(serversideCall).containsKeys("type", "id", "status"));
	...
	void aToolCallCanBeAnsweredByHandAndReplayedWithItsReasoning() {
		var options = responsesOptions().model("gpt-5.6-luna")
			.reasoningEffort("low")
	...
	/**
	 * A server-side tool and a local one in the same request. OpenAI runs the former
	 * itself, so it must not come back as something {@link ToolCallingAdvisor} tries to
	 * execute, while the local tool still round-trips through the advisor.
	 */
	@Test
	void serversideAndLocalToolsCoexistInOneRequest() {
		var options = responsesOptions().model("gpt-5.6-luna").serversideTools(ServersideTool.WebSearch.of()).build();
	...
	/**
	 * The other server-side tool that needs no external setup. The task is one no model
	 * answers from memory and no counting convention can shift - an off-by-one in how
	 * "the nth Fibonacci number" is indexed would fail this test for reasons that have
	 * nothing to do with the endpoint - and the prompt still has to insist on the tool,
	 * because a model may try to reason its way there instead.
	 */
	@Test
	void theCodeInterpreterRunsAtOpenAi() {
		var options = responsesOptions().model("gpt-5.6-luna")
			.serversideTools(ServersideTool.CodeInterpreter.of())
			.build();

		ChatResponse response = this.chatModel.call(new Prompt("Use the code interpreter to compute the sum "
				+ "of all prime numbers below 10000. Answer with the number only.", options));

		// The server-side call surfaces as metadata rather than as something to
		// execute, which is what this test is about; the number only proves its output
		// reached the answer
		assertThat(response.getResult()
			.getMetadata()
			.<List<Map<String, Object>>>get(OpenAiResponsesMetadata.SERVERSIDE_TOOL_CALLS))
			.as("transcript %s", response.getResult().getOutput().getParts())
			.anySatisfy(serversideCall -> assertThat(serversideCall).containsEntry("type", "code_interpreter_call"));
		assertThat(response.getResult().getOutput().getText()).containsAnyOf("5736396", "5,736,396");
	}
	...
	/**
	 * Deliberately {@link Configuration @Configuration} and not
	 * {@link org.springframework.boot.SpringBootConfiguration @SpringBootConfiguration}:
	 * this test lives in the same package as {@link OpenAiTestConfiguration}, and a
	 * second {@code @SpringBootConfiguration} there breaks every sibling test that has no
	 * configuration class of its own, because Boot then finds two while searching up the
	 * package hierarchy. Naming it in {@code @SpringBootTest(classes = ...)} is enough
	 * for it to be used here, and it is not {@code @TestConfiguration} so it replaces
	 * {@link OpenAiTestConfiguration} rather than being merged with it - which would
	 * leave two {@link OpenAiChatModel} beans to inject.
	 */
	...
	static class Config {
		...
		OpenAiChatModel openAiChatModel() {
			return OpenAiChatModel.builder().options(responsesOptions().build()).build();
		}
