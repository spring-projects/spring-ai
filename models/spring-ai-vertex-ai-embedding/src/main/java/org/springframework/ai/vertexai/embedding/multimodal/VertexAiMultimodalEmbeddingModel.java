...
import org.springframework.ai.vertexai.embedding.VertextAiEmbeddingUtility.ImageBuilder;
import org.springframework.ai.vertexai.embedding.VertextAiEmbeddingUtility.MultimodalInstanceBuilder;
import org.springframework.ai.vertexai.embedding.VertextAiEmbeddingUtility.VideoBuilder;
...
/**
 * Exmperimental implementation of the Vertex AI Multimodal Embedding Model. Note: This
 * implementation is not yet fully functional and is subject to change.
 *
...
public class VertexAiMultimodalEmbeddingModel implements DocumentEmbeddingModel {
	...
	public VertexAiMultimodalEmbeddingModel(VertexAiEmbeddigConnectionDetails connectionDetails,
			VertexAiMultimodalEmbeddingOptions defaultOpitons) {

		Assert.notNull(defaultOpitons, "VertexAiEmbeddingOptions must not be null");
		this.defaultOptions = defaultOpitons;
	...
	private EmbeddingResponse doSingleDocumentPrediction(PredictionServiceClient client, EndpointName endpointName,
		...
		Map<ModalityType, DocumentMetadata> documentMetadata = new HashMap<>();

		// optional dimiension parameter
		...
		if (!CollectionUtils.isEmpty(document.getMedia())) {
			...
			for (Media media : document.getMedia()) {
				if (media.getMimeType().isCompatibleWith(TEXT_MIME_TYPE)) {
					...
					if (StringUtils.hasText(document.getContent())) {
						logger.warn("Meida type String overrides the Document text content!");
					}
				}
		...
		List<Value> instances = List.of(VertextAiEmbeddingUtility.valueOf(instanceBuilder.build()));

		PredictRequest.Builder predictRequestBuilder = PredictRequest.newBuilder()
			.setEndpoint(endpointName.toString())
			.setParameters(VertextAiEmbeddingUtility.jsonToValue(ModelOptionsUtils.toJsonString(Map.of())))
		...
		for (Value prediction : embeddingResponse.getPredictionsList()) {
			if (prediction.getStructValue().containsFields("textEmbedding")) {
				...
				List<Double> textVector = VertextAiEmbeddingUtility.toVector(textEmbedding);
			...
			if (prediction.getStructValue().containsFields("imageEmbedding")) {
				...
				List<Double> imageVector = VertextAiEmbeddingUtility.toVector(imageEmbedding);
			...
			if (prediction.getStructValue().containsFields("videoEmbeddings")) {
				...
				if (videoEmbeddings.getListValue().getValues(0).getStructValue().containsFields("embedding")) {
					...
					List<Double> videoVector = VertextAiEmbeddingUtility.toVector(embeddings);
