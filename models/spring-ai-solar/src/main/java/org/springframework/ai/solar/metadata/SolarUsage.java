...
public class SolarUsage implements Usage {
	...
	private final SolarApi.ChatCompletion.Usage usage;

	protected SolarUsage(SolarApi.ChatCompletion.Usage usage) {
		Assert.notNull(usage, "Solar Usage must not be null");
		this.usage = usage;
	}

	public static SolarUsage from(SolarApi.ChatCompletion.Usage usage) {
		return new SolarUsage(usage);
	}

	protected SolarApi.ChatCompletion.Usage getUsage() {
		return this.usage;
	}
