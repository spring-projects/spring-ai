 ...
 */
@SpringBootTest(classes = PerplexityWithOpenAiSdkChatModelIT.Config.class)
@EnabledIfEnvironmentVariable(named = "PERPLEXITY_API_KEY", matches = ".+")
@Disabled("Requires Perplexity credits")
class PerplexityWithOpenAiSdkChatModelIT {

	private static final Logger logger = LoggerFactory.getLogger(PerplexityWithOpenAiSdkChatModelIT.class);
	...
	void streamingWithTokenUsage() {
		var promptOptions = OpenAiSdkChatOptions.builder()
			.streamOptions(OpenAiSdkChatOptions.StreamOptions.builder().includeUsage(true).build())
			.seed(1)
			.build();
	...
	void validateCallResponseMetadata() {
		ChatResponse response = ChatClient.create(this.chatModel)
			.prompt()
			.options(OpenAiSdkChatOptions.builder().model(DEFAULT_PERPLEXITY_MODEL).build())
