 ...
 */
@Testcontainers
@SpringBootTest(classes = OllamaWithOpenAiSdkChatModelIT.Config.class)
class OllamaWithOpenAiSdkChatModelIT {

	private static final Logger logger = LoggerFactory.getLogger(OllamaWithOpenAiSdkChatModelIT.class);
	...
	void multiModalityEmbeddedImage() {
		...
		var response = this.chatModel
			.call(new Prompt(List.of(userMessage), OpenAiSdkChatOptions.builder().model(MULTIMODAL_MODEL).build()));
	...
	static class Config {
		...
		public OpenAiSdkChatModel openAiSdkChatModel() {
			return OpenAiSdkChatModel.builder()
				.options(OpenAiSdkChatOptions.builder().baseUrl(baseUrl).model(DEFAULT_OLLAMA_MODEL).build())
				.build();
		}
