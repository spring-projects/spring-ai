...
import software.amazon.awssdk.regions.providers.DefaultAwsRegionProviderChain;import software.amazon.awssdk.services.bedrockruntime.BedrockRuntimeAsyncClient;
	...
	protected Flux<SO> internalInvocationStream(I request, Class<SO> clazz) {
		...
		InvokeModelWithResponseStreamResponseHandler.Visitor visitor = InvokeModelWithResponseStreamResponseHandler.Visitor
				...
				.onDefault(event -> {
					...
					eventSink.emitError(new Throwable("Unknown or unhandled event: " + event), DEFAULT_EMIT_FAILURE_HANDLER);
				})
				.build();
