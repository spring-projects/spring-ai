...
public class StabilityAiImageClient implements ImageClient {
	...
	/**
	 * Calls the StabilityAiImageClient with the given ImagePrompt and returns the
	 * ImageResponse.
	 * @see #call(StabilityAiImagePrompt) to pass in the full set of Prompt instructions
	 * that StabilityAI supports.
	 * @param prompt the ImagePrompt containing the prompt and image model options
	 * @return the ImageResponse generated by the StabilityAiImageClient
	 */
	@Override
	public ImageResponse call(ImagePrompt prompt) {
		// Convert the set of portable options contained in ImagePrompt to the provider
		// specific StabilityAI options
		StabilityAiImageOptions runtimeOptions = convertOptions(prompt.getOptions());
		// Merge the runtime options passed via the prompt with the options configured via
		// Autoconfiguration. Runtime options overwrite StabilityAiImageClient options
		...
		// Convert the portable prompt instructions contained in ImagePrompt to the
		// provider specific StabilityAI prompt data type
		List<StabilityAiImageMessage> messages = convertInstructions(prompt.getInstructions());
		// Call StabilityAI API
		return call(new StabilityAiImagePrompt(messages, optionsToUse));
	}
	 ...
	 * @param stabilityAiImagePrompt the StabilityAiImagePrompt containing the prompt and
	 * image model options
	 * @return the ImageResponse generated by the StabilityAiImageClient
	 */
	public ImageResponse call(StabilityAiImagePrompt stabilityAiImagePrompt) {

		StabilityAiImageOptions runtimeOptions = stabilityAiImagePrompt.getOptions();
		...
		// Copy the org.springframework.ai.model derived ModelPrompt and ModelOptions data
		// types to the data types used in StabilityAiApi
		// Perhaps StabilityAiImageClient can use these classes directly instead of
		// copying into other data types.
		StabilityAiApi.GenerateImageRequest generateImageRequest = getGenerateImageRequest(stabilityAiImagePrompt,
				optionsToUse);
		...
		// Convert to org.springframework.ai.model derived ModelResponse data type
		return convertResponse(generateImageResponse);

	}

	private static StabilityAiApi.GenerateImageRequest getGenerateImageRequest(
			StabilityAiImagePrompt stabilityAiImagePrompt, StabilityAiImageOptions optionsToUse) {
