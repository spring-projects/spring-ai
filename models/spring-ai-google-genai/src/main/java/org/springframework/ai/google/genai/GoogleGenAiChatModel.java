...
public class GoogleGenAiChatModel implements ChatModel, DisposableBean {
	...
	/**
	 * Metadata key under which the code generated by the Gemini code execution tool is
	 * exposed on the response {@link AssistantMessage}.
	 * @since 2.0.1
	 */
	public static final String METADATA_EXECUTABLE_CODE = "executableCode";

	/**
	 * Metadata key under which the language of the code generated by the Gemini code
	 * execution tool is exposed on the response {@link AssistantMessage}.
	 * @since 2.0.1
	 */
	public static final String METADATA_EXECUTABLE_CODE_LANGUAGE = "executableCodeLanguage";

	/**
	 * Metadata key under which the output produced by running the Gemini code execution
	 * tool is exposed on the response {@link AssistantMessage}.
	 * @since 2.0.1
	 */
	public static final String METADATA_CODE_EXECUTION_RESULT = "codeExecutionResult";

	/**
	 * Metadata key under which the outcome of the Gemini code execution tool is exposed
	 * on the response {@link AssistantMessage}.
	 * @since 2.0.1
	 */
	public static final String METADATA_CODE_EXECUTION_OUTCOME = "codeExecutionOutcome";
		...
		if (isFunctionCall) {
			...
			List<Generation> generations = candidate.content()
				...
				.map(part -> {
					...
					// Code execution parts are not part of the message text; expose them
					// as metadata.
					part.executableCode().ifPresent(executableCode -> {
						executableCode.code()
							.ifPresent(code -> partMessageMetadata.put(METADATA_EXECUTABLE_CODE, code));
						executableCode.language()
							.ifPresent(language -> partMessageMetadata.put(METADATA_EXECUTABLE_CODE_LANGUAGE,
									language.toString()));
					});
					part.codeExecutionResult().ifPresent(result -> {
						result.outcome()
							.ifPresent(outcome -> partMessageMetadata.put(METADATA_CODE_EXECUTION_OUTCOME,
									outcome.toString()));
						result.output()
							.ifPresent(output -> partMessageMetadata.put(METADATA_CODE_EXECUTION_RESULT, output));
					});
	...
	GeminiRequest createGeminiRequest(Prompt prompt) {
		...
		if (prompt.getOptions() instanceof GoogleGenAiChatOptions googleGenAiChatOptions
				&& Boolean.TRUE.equals(googleGenAiChatOptions.getCodeExecution())) {
			var codeExecution = ToolCodeExecution.builder().build();
			final var codeExecutionTool = Tool.builder().codeExecution(codeExecution).build();
