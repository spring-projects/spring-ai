...
public class MistralAiTestConfiguration {
	...
	public MistralAiApi mistralAiApi() {
		return new MistralAiApi(getApiKey());
	}

	private String getApiKey() {
		var apiKey = System.getenv("MISTRALAI_API_KEY");
		if (!StringUtils.hasText(apiKey)) {
			throw new IllegalArgumentException(
					"You must provide an API key.  Put it in an environment variable under the name MISTRALAI_API_KEY");
		}
		return apiKey;
	}
