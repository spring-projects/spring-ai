...
class AnthropicCacheOptionsTests {
	...
	void defaultsAreSane() {
		...
		for (MessageType mt : MessageType.values()) {
			assertThat(options.getMessageTypeTtls().get(mt)).isEqualTo(AnthropicCacheTtl.FIVE_MINUTES);
	...
	void builderOverrides() {
		...
		assertThat(options.getMessageTypeTtls().get(MessageType.SYSTEM)).isEqualTo(AnthropicCacheTtl.ONE_HOUR);
