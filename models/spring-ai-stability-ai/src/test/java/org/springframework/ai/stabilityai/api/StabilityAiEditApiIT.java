...
public class StabilityAiEditApiIT {
	...
	void removeBackgroundToRawImage() throws IOException {
		...
		// when
		StructuredResponse response = editApi.removeBackground(request).getBody();

		// then
		assertThat(response).isNotNull();
		assertThat(response.b64Image()).isNull();
		assertThat(response.rawImage()).isNotNull();
		assertThat(response.xRequestId()).isNotNull();
		assertThat(response.seed()).isNotNull();
		assertThat(response.finishReason()).isEqualTo(FinishReason.SUCCESS);
		assertThat(response.contentType()).isEqualTo("image/png");

		writeRawImageToFile(response.rawImage(), "png");
	}
	...
	void removeBackgroundToBase64Image() throws IOException {
		...
		// when
		StructuredResponse response = editApi.removeBackground(request, headers).getBody();

		// then
		assertThat(response).isNotNull();
		assertThat(response.b64Image()).isNotNull();
		assertThat(response.rawImage()).isNull();
		assertThat(response.xRequestId()).isNotNull();
		assertThat(response.seed()).isNotNull();
		assertThat(response.finishReason()).isEqualTo(FinishReason.SUCCESS);
