...
/**
 * Oracle dialect for batch execution repository.
 *
 * @author Yasin Akbas
 * @since 2.0.0
 */
public class OracleBatchExecutionRepositoryDialect implements JdbcBatchExecutionRepositoryDialect {

	@Override
	public String getUpsertSql() {
		return """
				MERGE INTO SPRING_AI_BATCH_EXECUTION target
				USING (SELECT ? AS batch_id, ? AS endpoint, ? AS status, ? AS request_count, ? AS input_file_id, ? AS created_at, ? AS updated_at FROM DUAL) source
