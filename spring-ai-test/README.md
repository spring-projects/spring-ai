...
class MyAiEvaluationTest {
    ...
    void testQuestionAnswerAccuracy() {
        ...
        Evaluator evaluator = FactCheckingEvaluator.builder(chatClientBuilder).build();
...
The test requires:
- A `ChatClient.Builder` bean backed by a configured `ChatModel`
- Any evaluator-specific model, prompt, or options configuration required by the selected `Evaluator`
