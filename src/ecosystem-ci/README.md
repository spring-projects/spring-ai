...
| Condition | Action |
|-----------|--------|
| Dependency fails (day 1-2) | Dashboard updated, no alert |
| Dependency fails for 3+ days | Alert comment posted (triggers notification) |
| Dependency recovers | Dashboard updated, no alert |

The `alert_after_days` setting in `ci-alert-config.json` controls the threshold (default: 3 days).
...
| `alert_after_days` | Days of failure before alerting | 3 |
