...
import org.springframework.ai.chat.memory.AdvancedChatMemoryRepository;
...
/**
 * Integration tests for RedisChatMemory advanced query capabilities.
 *
...
class RedisChatMemoryAdvancedQueryIT {
	...
	void shouldFindMessagesByType_singleConversation() {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> userMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> systemMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> assistantMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> toolMessages = ((AdvancedChatMemoryRepository) chatMemory)
	...
	void shouldRespectLimitParameter() {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> messages = ((AdvancedChatMemoryRepository) chatMemory)
	...
	void shouldHandleToolMessages() {
		this.contextRunner.run(context -> {
			...
			ToolResponseMessage.ToolResponse toolResponse = new ToolResponseMessage.ToolResponse("tool-1", "weather",
					"{\"temperature\":\"22°C\"}");
			ToolResponseMessage toolMessage = new ToolResponseMessage(List.of(toolResponse));
	...
	void shouldFindMessagesByContent() {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> programmingMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> javaMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> springMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			// Test with limit
			List<AdvancedChatMemoryRepository.MessageWithConversation> limitedMessages = ((AdvancedChatMemoryRepository) chatMemory)
	...
	void shouldFindMessagesByTimeRange() throws InterruptedException {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> allMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> firstHalfMessages = ((AdvancedChatMemoryRepository) chatMemory)
				.findByTimeRange(null, java.time.Instant.ofEpochMilli(startTime),
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> conv2Messages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			// Test with limit
			List<AdvancedChatMemoryRepository.MessageWithConversation> limitedTimeMessages = ((AdvancedChatMemoryRepository) chatMemory)
				.findByTimeRange(null, java.time.Instant.ofEpochMilli(startTime),
	...
	void shouldFindMessagesByMetadata() {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> highPriorityMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> questionMessages = ((AdvancedChatMemoryRepository) chatMemory)
				.findByMetadata("category", "question", 10);

			assertThat(questionMessages).hasSize(2);

			// Test finding by numeric metadata
			List<AdvancedChatMemoryRepository.MessageWithConversation> highScoreMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> confidentMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> nonExistentMessages = ((AdvancedChatMemoryRepository) chatMemory)
	...
	void shouldExecuteCustomQuery() {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> redisUserMessages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> conv1Messages = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> complexResults = ((AdvancedChatMemoryRepository) chatMemory)
				.executeQuery(complexQuery, 10);

			assertThat(complexResults).hasSize(2);

			// Test with limit
			List<AdvancedChatMemoryRepository.MessageWithConversation> limitedResults = ((AdvancedChatMemoryRepository) chatMemory)
				.executeQuery("*", 2);
	...
	void shouldHandleSpecialCharactersInQueries() {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> plusMessages = ((AdvancedChatMemoryRepository) chatMemory)
				.findByContent("C++", 10);
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> colonMessages = ((AdvancedChatMemoryRepository) chatMemory)
	...
	void shouldReturnEmptyListForNoMatches() {
		this.contextRunner.run(context -> {
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> noContentMatch = ((AdvancedChatMemoryRepository) chatMemory)
				.findByContent("nonexistent", 10);
			assertThat(noContentMatch).isEmpty();

			// Test time range with no messages
			List<AdvancedChatMemoryRepository.MessageWithConversation> noTimeMatch = ((AdvancedChatMemoryRepository) chatMemory)
			...
			List<AdvancedChatMemoryRepository.MessageWithConversation> noMetadataMatch = ((AdvancedChatMemoryRepository) chatMemory)
				.findByMetadata("nonexistent", "value", 10);
			assertThat(noMetadataMatch).isEmpty();

			// Test custom query with no matches
			List<AdvancedChatMemoryRepository.MessageWithConversation> noQueryMatch = ((AdvancedChatMemoryRepository) chatMemory)
