...
/**
 * Integration tests for RedisChatMemory focusing on different message types.
 *
...
class RedisChatMemoryMessageTypesIT {
	...
	void shouldHandleAssistantMessageWithToolCalls() {
		this.contextRunner.run(context -> {
			...
			AssistantMessage assistantMessage = new AssistantMessage("I'll check that for you.",
					Map.of("model", "gpt-4"), toolCalls, List.of());
	...
	void shouldHandleBasicToolResponseMessage() {
		this.contextRunner.run(context -> {
			...
			ToolResponseMessage toolResponseMessage = new ToolResponseMessage(List.of(weatherResponse));
	...
	void shouldHandleToolResponseMessageWithMultipleResponses() {
		this.contextRunner.run(context -> {
			...
			ToolResponseMessage toolResponseMessage = new ToolResponseMessage(
					List.of(weatherResponse, calculatorResponse, databaseResponse),
					Map.of("source", "tools-api", "version", "1.0"));
	...
	void getMessages_withAllMessageTypes_shouldPreserveMessageOrder() {
		this.contextRunner.run(context -> {
			...
			AssistantMessage assistantToolCall = new AssistantMessage("I'll check the weather in Paris for you.",
					Map.of(), toolCalls, List.of());
