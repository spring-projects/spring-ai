...
/**
 * Integration tests for RedisChatMemory to verify proper handling of Media content.
 *
...
class RedisChatMemoryMediaIT {
	...
	void shouldStoreAndRetrieveAssistantMessageWithByteArrayMedia() {
		this.contextRunner.run(context -> {
			...
			AssistantMessage assistantMessage = new AssistantMessage("Response with image",
					Map.of("assistant-key", "assistant-value"), toolCalls, List.of(byteArrayMedia));
	...
	void shouldStoreAndRetrieveMultipleMessagesWithDifferentMediaTypes() {
		this.contextRunner.run(context -> {
			...
			AssistantMessage assistantMessage = new AssistantMessage("Response with JPEG", Map.of(), List.of(),
					List.of(jpegMedia));
	...
	void shouldHandleComplexBinaryDataTypes() {
		this.contextRunner.run(context -> {
			...
			AssistantMessage assistantMessage = new AssistantMessage("Response with video attachment", Map.of(),
					List.of(), List.of(videoMedia));
