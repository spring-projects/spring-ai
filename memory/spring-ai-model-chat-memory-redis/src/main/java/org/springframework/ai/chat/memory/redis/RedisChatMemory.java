 ...
 */
public final class RedisChatMemory implements ChatMemory, ChatMemoryRepository, AdvancedChatMemoryRepository {
		...
		result.getDocuments().forEach(doc -> {
			if (doc.get("$") != null) {
				...
				if (MessageType.ASSISTANT.toString().equals(type)) {
					...
					AssistantMessage assistantMessage = new AssistantMessage(content, metadata, toolCalls, media);
					...
					messages.add(new ToolResponseMessage(toolResponses, metadata));
				}
	...
	public static class Builder {
		 ...
		 * @return this builder
		 */
		public Builder ttlSeconds(long timeToLiveSeconds) {
		...
		/**
		 * Builds and returns an instance of {@link RedisChatMemory}.
		 * @return a new {@link RedisChatMemory} instance
		 */
		public RedisChatMemory build() {
			...
			return new RedisChatMemory(config);
		}
