...
. Split up the document into parts while preserving the semantic boundaries of the content.
...
. Split up the document's parts further into parts whose size is a small percentage of the AI Model's token limit.
...
* `DocumentTransformer`: Responsible for processing the data in various ways (for example, splitting up documents into smaller pieces or adding additional metadata to the `Document`.)
