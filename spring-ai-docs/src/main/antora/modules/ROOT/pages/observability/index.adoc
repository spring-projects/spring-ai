...
The `spring.ai.tool` observations are recorded when performing tool calling in the context of a chat model interaction, and when executing MCP server `tools/call` handlers registered through the Spring Boot MCP server auto-configurations. They measure the time spent on tool call completion and propagate the related tracing information.

For MCP server tool calls, protocol-level error results (`CallToolResult.isError() == true`) are recorded as observation errors. Success and failure can be derived from the observation metric count and its standard `error` tag.
...
|`spring.ai.mcp.server.protocol` | MCP server protocol used to handle the tool call. It's `stateful` or `stateless`. Only present for MCP server tool calls.
|`spring.ai.mcp.server.type` | MCP server execution type used to handle the tool call. It's `sync` or `async`. Only present for MCP server tool calls.
|===
