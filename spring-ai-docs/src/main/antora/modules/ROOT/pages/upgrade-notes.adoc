...
=== Redis Chat Memory: Deprecated Module and Legacy Prefix Removed

The transitional compatibility introduced in 2.0.1 has been removed.

==== Impact

* The `spring-ai-autoconfigure-model-chat-memory-redis` artifact no longer exists. Consumers must depend on `spring-ai-autoconfigure-model-chat-memory-repository-redis` (or the `spring-ai-starter-model-chat-memory-repository-redis` starter).
* The legacy configuration prefix `spring.ai.chat.memory.redis.*` is no longer bound. Use `spring.ai.chat.memory.repository.redis.*` instead.
* The deprecated `RedisChatMemoryAutoConfiguration` and `RedisChatMemoryProperties` types (in the `org.springframework.ai.model.chat.memory.redis.autoconfigure` package) have been deleted.

==== Migration

Follow the 2.0.1 xref:upgrading-to-2-0-1[migration steps] below if you have not done so already; the transitional fallbacks described there are no longer applied at runtime.

=== Message Parts

==== New: Ordered `MessagePart` Content on `AssistantMessage` and `UserMessage`

`AssistantMessage` and `UserMessage` now hold their content as an ordered list of `MessagePart` entries (`TextPart`, `ReasoningPart`, `ToolCallPart`, `ToolResultPart`, `MediaPart`, `UnknownPart`), available through `getParts()`.
`getText()`, `getMedia()`, `getToolCalls()` and the new `getReasoning()` are views over the parts.
See xref:api/chatmodel.adoc#message-parts[Message Parts].

===== Impact

The constructors and the existing builder methods keep working and produce the parts in the order text, tool calls, media, so no code change is required.

* `getMedia()` and `getToolCalls()` return unmodifiable lists; add media and tool calls through the builder instead of mutating the returned list.
* `equals()` and `hashCode()` of both message types compare the parts, so media now takes part in equality; `Media` gained value equality over id, MIME type and data (the name is excluded because it defaults to a random value).
* A message with parts but no text part has an empty text; `getText()` is `null` only for a message without any part.

===== Deprecated

The following are still written or accepted for one release and will be removed afterwards.

* The protected `textContent` field for all subclasses of AbstractMessage, is kept, but should be accessed via `getText()`
* The protected `media` field of `AssistantMessage` and `UserMessage`, kept for subclasses that read it.
* The protected `responses` field of `ToolResponseMessage`, kept for subclasses that read it.


==== New: Streaming Contract for Parts

Streamed chunks carry indexed partial parts (`StreamingParts`) and the response id, and `MessageAggregator` rebuilds the complete parts from them.
Raw `ChatModel.stream()` subscribers that only read `getText()` see the text deltas exactly as before.
Reasoning deltas now carry their text in a `ReasoningPart`.

===== Impact

A streamed chunk never has a `null` text, so `map(AssistantMessage::getText).collect(Collectors.joining())` keeps working.
By default, tool calls are still buffered and arrive once as a complete call.
Partial parts are not tool calls, so nothing executes on them, and `ChatClient` hides them either way.
...
The artifact ID and configuration properties of the Redis chat memory auto-configurations module has changed:

* *Artifact:* `spring-ai-autoconfigure-model-chat-memory-redis` → `spring-ai-autoconfigure-model-chat-memory-repository-redis`
* *Configuration properties:* `spring.ai.chat.memory.redis.*` → `spring.ai.chat.memory.repository.redis.*`

==== Migration

- Update your Maven/Gradle dependencies to use the new artifact ID:
...
- Update your configuration properties to use the new prefix:
...
The new module provided a smooth transition from the deprecated module in the 2.0.1 release line:

- The deprecated `spring-ai-autoconfigure-model-chat-memory-redis` module still existed and depended on the new module via a transitive dependency, so existing dependencies continued to work without changes.
- The deprecated `RedisChatMemoryProperties` (`spring.ai.chat.memory.redis`) delegated every property to the current `RedisChatMemoryRepositoryProperties` (`spring.ai.chat.memory.repository.redis`), the same delegation mechanism used for the `.options` deprecation further down in this page, so existing property configurations continued to work.

NOTE: Both the deprecated module and the legacy configuration prefix have been removed in 2.1.0. See xref:upgrading-to-2-1-0[Upgrading to 2.1.0].

=== Mistral AI Moderation Categories Restructured

The Mistral AI moderation model has migrated from `mistral-moderation-2411` (retired June 30, 2026) to `mistral-moderation-2603`.
As part of this migration, the single `dangerousAndCriminalContent` category has been split into two distinct categories: `dangerous` and `criminal`.
A new `jailbreaking` category has also been added.

==== Impact

The following methods on `Categories` and `CategoryScores` are deprecated and will be removed in a future release:
...
Replace usages of the deprecated `dangerousAndCriminalContent` methods with the new `criminal` and `dangerous` equivalents:
...
You can also check the new `jailbreaking` category, which has no prior equivalent:
...
=== Tool Resolution Fallback Disabled by Default

The fallback that resolves and executes tools registered as beans, but not attached to a request has been disabled by default.
Previously, when the model returned a tool call for a tool that was not attached to the request, `DefaultToolCallingManager` fell back to the application-wide `ToolCallbackResolver` (backed by every `ToolCallback` bean in the application context) and executed the resolved tool.
Starting with 2.0.1, the tools attached to a request — via `.tools(...)`, `.defaultTools(...)`, or `ToolCallingChatOptions.toolCallbacks(...)` — are the only tools that can be executed for that request.

To restore the previous behavior, re-enable the resolution fallback:
...
When constructing a `ToolCallingManager` directly, use the builder switch instead:
...
See xref:api/tools.adoc#tool-resolution[Tool Resolution] for details.
...
For a complete example including the streaming path, see xref:api/tools.adoc#user-controlled-tool-execution[User-Controlled Tool Execution — With ChatClient].

==== Removed: Internal Tool Execution Loop in `ChatModel`

In Spring AI 1.x, every `ChatModel` implementation contained its own internal tool-execution loop: calling `chatModel.call(prompt)` with tools would execute any tool calls in the response and re-call the model automatically, looping until the model produced a non-tool-call response.

In 2.0, this internal loop has been removed from every `ChatModel` implementation (OpenAI, Anthropic, Ollama, DeepSeek, Bedrock, MiniMax, Mistral, Google GenAI). `ChatModel.call(prompt)` and `ChatModel.stream(prompt)` now return the model's raw response — tool calls are not executed automatically.

===== Impact

Code that called `ChatModel` directly with tools and relied on automatic execution now receives the model's tool call request in the response without it being executed.

===== Migration

For most applications, switch to `ChatClient`. The auto-registered `ToolCallingAdvisor` runs the loop for you:
...
When you genuinely need `ChatModel` directly (custom orchestrator, infrastructure code), drive the loop manually with `ToolCallingManager`. See xref:api/tools/chatmodel-tool-calling.adoc#driving-the-loop-manually[ChatModel Tool Calling: Driving the Loop Manually].

==== Removed: `SpringBeanToolCallbackResolver` and `toolNames()`

`SpringBeanToolCallbackResolver` and the `toolNames()` API — which resolved tools from bare `Function`/`Supplier`/`Consumer` beans by name — have been removed. Tools must now be registered as explicit `ToolCallback` instances or `@Tool`-annotated objects.

===== Impact

Code that declared `Function` beans and referenced them by name via `toolNames(...)` will no longer compile or resolve.

===== Migration

Declare a `ToolCallback` bean directly (for function-style tools, use `FunctionToolCallback.builder()`):
...
Method-based tools can also be declared with `@Tool`:
...
The recursive tool-calling advisor class has been renamed for precision. If you referenced `ToolCallAdvisor` directly (for example, by registering a custom instance or extending it), update the reference to `ToolCallingAdvisor`.
...
==== Renamed: `FunctionCallback` API → `ToolCallback` API

Spring AI 1.0 introduced `ToolCallback` as the replacement for the older `FunctionCallback` API; 2.0 removes the deprecated `FunctionCallback` types and methods entirely. The terminology is now consistent — "tools" everywhere, "functions" nowhere.

If you're upgrading from a version that still used the legacy `FunctionCallback` API:
...
| `FunctionCallback.builder().method(...)` | `MethodToolCallback.builder()` with `ToolDefinition`
...
[source,java]
----
// Before — function-style callback
...
// After — explicit ToolCallback via FunctionToolCallback
...
[source,java]
----
// Before — defaultFunctions in builder
...
[source,java]
----
// Before — per-call functions()
chatClient.prompt()
    .user("...")
    .functions(callback)
    .call();

// After — per-call tools()
chatClient.prompt()
    .user("...")
    .tools(callback)
    .call();
...
Minimax dedicated support has been removed in favor of using Anthropic support as recommended by https://platform.minimax.io/docs/api-reference/text-anthropic-api[Minimax themselves].
...
Remove the explicit `ToolCallingAdvisor` from the chain and let auto-registration handle it. If you need a custom configuration, either supply a pre-configured `ToolCallingAdvisor.Builder` at `ChatClient` construction time (see the xref:api/chatclient.adoc#_customizing_the_default_toolcallingadvisor[Customizing the Default ToolCallingAdvisor] section), or disable auto-registration and register the advisor manually:
...
NOTE: This restriction applies to `ChatClient` only. When calling `ChatModel.call(Prompt)` directly, the prompt must carry a fully built `ChatOptions` instance of the concrete type expected by the model (_e.g._ `AnthropicChatOptions` for `AnthropicChatModel`). The `ChatModel` does **not** merge options: if `Prompt.getOptions()` is non-null it is used as-is; if it is `null`, the model's own default options are used instead. No partial merging occurs at the `ChatModel` level.
...
The related modules can now be found at https://github.com/oracle/spring-cloud-oracle/tree/main/spring-ai-oracle.
...
NOTE: If you already migrated to the `McpClientCustomizer<B>` API, no further action is required.
...
`spring-ai-anthropic` also gains four new chat options. The full reference for each is in xref:api/chat/anthropic-chat.adoc[Anthropic Chat]; the highlights:
...
NOTE: If you are upgrading from `1.1.x` (or `1.0.x`), see xref:api/chat/anthropic-migration.adoc[Migrating the Anthropic Module to the Official Java SDK] for the full `RestClient` → SDK transition guide.
...
made available for runtime dynamic resolution (see xref:api/tools.adoc#tool-resolution[Tool Resolution]), but never
...
when explicitly requested, you can use one of the strategies described in xref:api/tools.adoc#tool-resolution[Tool Resolution].
