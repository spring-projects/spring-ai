...
NOTE: This restriction applies to `ChatClient` only. When calling `ChatModel.call(Prompt)` directly, the prompt may carry a fully built `ChatOptions` instance — `ChatModel.buildRequestPrompt()` converts it internally via `.mutate()` and merges it with the model defaults.
