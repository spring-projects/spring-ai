...
* **`ToolCallAdvisor` via `ChatClient`** (recommended) — auto-registered when tools are present; no flag needed.
* **User-controlled loop via `ChatModel` directly** — simply invoke `ChatModel` without `ToolCallAdvisor`. Tool calls will not be executed automatically; check `chatResponse.hasToolCalls()` yourself and drive the loop with `ToolCallingManager`.
...
If you implemented `ToolExecutionEligibilityPredicate` to customize when tool execution fires, migrate to `ToolExecutionEligibilityChecker` (see below) and supply it to `ToolCallAdvisor`.

==== New: `ToolExecutionEligibilityChecker` in `ToolCallAdvisor`

`ToolExecutionEligibilityChecker` (`Function<ChatResponse, Boolean>`) can now be set on `ToolCallAdvisor.Builder` to customize when the tool-call loop iterates. The default is `chatResponse -> chatResponse != null && chatResponse.hasToolCalls()`.
...
Spring Boot users can provide a `ToolExecutionEligibilityChecker` bean; it will be picked up automatically by the auto-configured `ToolCallAdvisor.Builder`:
...
A new `spring.ai.chat.client.tool-calling.enabled` property (default `true`) controls whether the `ToolCallAdvisor` is auto-registered by the auto-configured `ChatClient`.
...
This is a global alternative to using `AdvisorParams.toolCallAdvisorAutoRegister(false)` on every individual call.

=== JDBC Chat Memory `sequence_id` Column

The `JdbcChatMemoryRepository` schema replaces the `timestamp` column with a `sequence_id BIGINT` column. The column never held meaningful temporal data; it was used solely to preserve message order within a conversation. Relying on `TIMESTAMP` precision for ordering caused incorrect ordering on databases whose default `TIMESTAMP` precision is one second (notably MySQL and MariaDB). A plain integer sequence orders identically across every supported database.

===== Impact

- The `SPRING_AI_CHAT_MEMORY` table now defines `sequence_id BIGINT NOT NULL` instead of a `timestamp` column, and the index is renamed to `SPRING_AI_CHAT_MEMORY_CONVERSATION_ID_SEQUENCE_ID_IDX`.
- This is a breaking schema change. Existing tables created by Spring AI 1.x are not compatible and must be migrated or recreated.

===== Migration

Chat memory holds recent conversation context rather than permanent history, so the simplest path is to drop and recreate the table using the updated schema script for your database (`classpath:org/springframework/ai/chat/memory/repository/jdbc/schema-<platform>.sql`). Any in-flight conversations are lost, but new conversations use the corrected schema:

[source,sql]
----
DROP TABLE SPRING_AI_CHAT_MEMORY;
-- then let schema initialization recreate it, or run the 2.0 schema-<platform>.sql manually
----

To preserve existing conversations, perform an in-place migration that backfills `sequence_id` from the previous ordering. For PostgreSQL:
...
ALTER TABLE SPRING_AI_CHAT_MEMORY DROP COLUMN "timestamp";

DROP INDEX IF EXISTS SPRING_AI_CHAT_MEMORY_CONVERSATION_ID_TIMESTAMP_IDX;
CREATE INDEX SPRING_AI_CHAT_MEMORY_CONVERSATION_ID_SEQUENCE_ID_IDX
ON SPRING_AI_CHAT_MEMORY(conversation_id, sequence_id);
----

Adapt the identifier quoting and the row-identity expression (`ctid` above) to your database. The `ROW_NUMBER() OVER (PARTITION BY conversation_id ORDER BY <old timestamp>)` pattern reproduces the original per-conversation order on every supported engine.
...
Building on the changes introduced in 2.0.0-M6 for Chat models, the configuration properties for all other models (Embedding, Image, Audio, Moderation, OCR, etc.) no longer use the `.options` prefix. 
...
Embeddings are not support anymore.
...
==== New: Automatic `ToolCallAdvisor` Registration

`ChatClient` now automatically registers a `ToolCallAdvisor` in the advisor chain whenever tools are configured on a call (via `.tools(...)`, `.toolCallbacks(...)`, or defaults set on the builder). Previously, callers had to add `ToolCallAdvisor` manually.

===== Impact

Existing code that already adds a `ToolCallAdvisor` explicitly will have the advisor present in the chain twice — once from the explicit add and once from auto-registration.

===== Migration

Remove the explicit `ToolCallAdvisor` from the chain and let auto-registration handle it. If you need a custom configuration, either supply a pre-configured `ToolCallAdvisor.Builder` at `ChatClient` construction time (see the xref:api/chatclient.adoc#_customizing_the_default_toolcalladvisor[Customizing the Default ToolCallAdvisor] section), or disable auto-registration and register the advisor manually:
...
A `ToolAdvisor` marker interface has been added. `ToolCallAdvisor` implements it. `DefaultChatClient` checks for this marker to decide whether to auto-register a `ToolCallAdvisor` — if any advisor in the chain already implements `ToolAdvisor`, auto-registration is skipped.

Custom advisors that own the tool-call lifecycle should implement `ToolAdvisor` to prevent a duplicate `ToolCallAdvisor` from being added automatically.

==== New: `MemoryAdvisor` Marker Interface

A `MemoryAdvisor` marker interface has been added. `BaseChatMemoryAdvisor` now extends it. `DefaultChatClient` uses this marker to detect downstream memory advisors and disable the `ToolCallAdvisor`'s internal conversation history when one is present.
...
==== New: `ChatClient.builder()` Overload for Custom `ToolCallAdvisor` Configuration

A new 5-argument `ChatClient.builder()` static method and a matching `DefaultChatClientBuilder` constructor accept a `ToolCallAdvisor.Builder<?>` to configure the advisor used during auto-registration:

[source,java]
----
ChatClient chatClient = ChatClient
    .builder(chatModel, observationRegistry, null, null,
            ToolCallAdvisor.builder().toolCallingManager(myToolCallingManager))
    .build();
----
...
[[upgrading-to-2-0-0-M6-chat-memory]]
== Upgrading to 2.0.0-M6
