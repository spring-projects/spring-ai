...
The configuration properties do not use anymore `.options`, so for example the former `spring.ai.openai.chat.options.model` is now `spring.ai.openai.chat.model`.
