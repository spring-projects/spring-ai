...
The artifact ID, package, and configuration properties of the Redis chat memory auto-configurations module has changed:

* *Artifact:* `spring-ai-autoconfigure-model-chat-memory-redis` → `spring-ai-autoconfigure-model-chat-memory-repository-redis`
* *Package:* `org.springframework.ai.model.chat.memory.redis.autoconfigure` → `org.springframework.ai.model.chat.memory.redis.repository.autoconfigure`
...
- Update your imports and class references to use the new package:
...
- The deprecated `spring-ai-autoconfigure-model-chat-memory-redis` module still exists and delegates to the new module via transitive dependency, so existing dependencies continue to work without changes.
- The `RedisChatMemoryRepositoryAutoConfiguration` binds configuration properties from the new prefix (`spring.ai.chat.memory.repository.redis`) and falls back to the legacy prefix (`spring.ai.chat.memory.redis`) if the new prefix is not bound, so existing property configurations continue to work.

NOTE: Both the deprecated module and the legacy configuration prefix fallback will be removed in a future release.
