...
The new module provides a smooth transition from the deprecated module:

- The deprecated `spring-ai-autoconfigure-model-chat-memory-redis` module still exists and depends on the new module via a transitive dependency, so existing dependencies continue to work without changes.
- The deprecated `RedisChatMemoryProperties` (`spring.ai.chat.memory.redis`) delegates every property to the current `RedisChatMemoryRepositoryProperties` (`spring.ai.chat.memory.repository.redis`), the same delegation mechanism used for the `.options` deprecation further down in this page, so existing property configurations continue to work.
