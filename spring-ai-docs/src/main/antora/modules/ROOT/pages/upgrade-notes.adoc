...
==== PromptChatMemoryAdvisor: Behavior Changes and Migration

===== Memory Encoding Changed

The way `PromptChatMemoryAdvisor` encodes conversation history in the system prompt has changed.
Each message is now wrapped in an XML tag with its role as the tag name, and the message content is HTML-entity-escaped:

[source,text]
----
<user>user message text</user>
<assistant>assistant response text</assistant>
----

Previously, the encoding used plain-text role prefixes with no escaping:
...
The system prompt section label has also been updated to make the expected handling of the memory block explicit.

===== Custom System Prompt Template No Longer Supported

The `systemPromptTemplate()` builder method on `PromptChatMemoryAdvisor` is deprecated and now throws `UnsupportedOperationException` immediately.
Only the built-in default template, which uses the XML encoding described above, is supported.

If you passed a custom `PromptTemplate` to the builder, the call will fail at startup.
Migrate to `MessageChatMemoryAdvisor` instead (see below).

===== Migration to MessageChatMemoryAdvisor

`PromptChatMemoryAdvisor` has been deprecated since 1.1.3.
`MessageChatMemoryAdvisor` is the recommended replacement: it passes conversation history as typed message objects at the API level, rather than serialising it into the system prompt.
...
The `ChatClient` wiring and the `ChatMemory.CONVERSATION_ID` advisor parameter are identical for both advisors.
No other application code changes are required.

=== VectorStoreChatMemoryAdvisor: Memory Entry Format Changed

The format used to embed retrieved conversation history in the system prompt has changed.
Each memory entry is now wrapped in a typed `<memory-entry>` XML element with its content XML-escaped:

[source,text]
----
<memory-entry type="user">user message text</memory-entry>
<memory-entry type="assistant">assistant response text</memory-entry>
----

Previously, entries were concatenated as plain text with no escaping or structural markers:
...
The default system prompt template also now includes the following instruction:
...
Integration tests or custom logic that parses or matches the exact text of the system prompt will need to be updated to account for the new `<memory-entry>` wrapper elements and XML escaping.

==== Migration

No application code changes are required.
If you have integration tests that assert on the exact system prompt content, update the expected strings to use the `<memory-entry type="...">` format and include the new instruction line.

If the security controls are insufficient for your use case (for example, an agentic configuration with broad tool access), consider migrating to `MessageChatMemoryAdvisor`, which passes conversation history as typed `Message` objects and does not interpolate user-sourced content into the system prompt.
