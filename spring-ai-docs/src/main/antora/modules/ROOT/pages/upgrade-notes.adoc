...
==== `ChatModel` Options Replacing Instead of Merging

`ChatModel` options handling change merged by mistake in `2.0.0-M6` has been reverted to replacing instead of merging.
Out-of-the box options merging is supported at the `ChatClient` level, but not at the `ChatModel` level where that's still possible but needs to be handled manually.

The logic supported at the `ChatModel` level in 2.0 is:
...
It was previously promoted by mistake and has been restored to its private or package-private visibility.

==== Fixed Binding Failure for `spring.ai.ollama.chat.think`

`spring.ai.ollama.chat.think-option` property has been renamed to `spring.ai.ollama.chat.think`.
...
==== New: Automatic `ToolCallingAdvisor` Registration

`ChatClient` now automatically registers a `ToolCallingAdvisor` in the advisor chain whenever tools are configured on a call (via `.tools(...)`, `.toolCallbacks(...)`, or defaults set on the builder). Previously, callers had to add `ToolCallingAdvisor` manually.
