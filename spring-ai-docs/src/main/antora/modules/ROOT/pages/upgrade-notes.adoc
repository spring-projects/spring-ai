...
- `ChatOptions#copy()` has been removed because options are now strictly immutable.
- Modifying collections returned by options getters will throw an `UnsupportedOperationException`.

===== Migration

To create a modified copy of an options instance, use the `mutate()` method instead of `copy()`:

[source,java]
----
// Before
ChatOptions options = originalOptions.copy();

// After
ChatOptions options = originalOptions.mutate().build();
----
