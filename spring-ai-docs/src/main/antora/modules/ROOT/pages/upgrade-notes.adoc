...
==== Removed: `spring.ai.openai.chat.completions-path`

The `spring.ai.openai.chat.completions-path` property has been removed as the new `openai-java` SDK automatically appends the correct paths (like `/chat/completions`) to the base URL. If you were using a custom completions path (for example, with a local Docker Model Runner), you should update your `spring.ai.openai.chat.base-url` property to include any required API prefixes (such as `/v1`).
