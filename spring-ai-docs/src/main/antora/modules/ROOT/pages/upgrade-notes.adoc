...
When you genuinely need `ChatModel` directly (custom orchestrator, infrastructure code), drive the loop manually with `ToolCallingManager`. See xref:api/tools/chatmodel-tool-calling.adoc#_driving_the_loop_manually[ChatModel Tool Calling: Driving the Loop Manually].
...
For new code, prefer the declarative `@Tool` annotation — see xref:api/tools.adoc#_defining_tools[Defining Tools].
