...
The `org.springaicommunity:mcp-annotations` external library has been removed as a dependency of the `mcp-annotations-spring` module.
...
<plugin>
    ...
    <configuration>
        ...
        <recipes>
            <recipe>org.springframework.ai.migration.M3MigrateMcpAnnotations</recipe>
        </recipes>
        <recipeArtifactCoordinates>
            <recipeArtifactCoordinate>org.openrewrite:rewrite-java:LATEST</recipeArtifactCoordinate>
            <recipeArtifactCoordinate>org.openrewrite.recipe:rewrite-maven:LATEST</recipeArtifactCoordinate>
        </recipeArtifactCoordinates>
    </configuration>
</plugin>
----

Place the recipe file https://github.com/spring-projects/spring-ai/blob/main/src/rewrite/migrate-to-2-0-0-M3.yaml[`src/rewrite/migrate-to-2-0-0-M3.yaml`] in your project root (or on the classpath), then run:
