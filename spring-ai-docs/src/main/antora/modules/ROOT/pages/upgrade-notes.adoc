...
* The `SpeechModel` API, previously living in the `spring-ai-openai` module, has been generalized
and consolidated in the `spring-ai-core` module, ready for supporting more implementations.
As a consequence, the `OpenAiSpeechModel` implementation has been refactored to implement the new APIs.

== Upgrading to 1.0.0.M3
