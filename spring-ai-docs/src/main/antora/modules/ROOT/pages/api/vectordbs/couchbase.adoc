...
This section will walk you through setting up the `CouchbaseVectorStore` to store document embeddings and perform similarity searches using Couchbase.
...
Now you can auto-wire the `CouchbaseVectorStore` as a vector store in your application.
...
To connect to Couchbase and use the `CouchbaseVectorStore`, you need to provide access details for your instance.
...
Spring Boot's auto-configuration feature for the Couchbase Cluster will create a bean instance that will be used by the `CouchbaseVectorStore`.
...
Properties starting with the `spring.ai.vectorstore.couchbase.*` prefix are used to configure `CouchbaseVectorStore`.
...
More details about each in the https://docs.couchbase.com/server/current/search/child-field-options-reference.html[Couchbase Documentation] on vector searchs.
...
Instead of using the Spring Boot auto-configuration, you can manually configure the Elasticsearch vector store. For this you need to add the `spring-ai-couchbase-store` to your project:
...
and then create the `CouchbaseVectorStore` bean:

[source,java]
----
@Bean
public CouchbaseVectorStore vectorStore(Cluster cluster, EmbeddingModel embeddingModel) {
