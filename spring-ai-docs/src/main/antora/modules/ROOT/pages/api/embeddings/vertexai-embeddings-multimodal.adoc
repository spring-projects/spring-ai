...
NOTE: EXPERIMENTAL. Used for experimental peruposes only. Not compatible yet with the `VectorStores`.
...
NOTE: The VertexAI Multimodal API imposes the link:https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-multimodal-embeddings#api-limits[following lmits].
...
var document = new Document("Explain what do you see on this vidoe?", List.of(imageMedial, videoMedial), Map.of());
