...
For the conceptual overview of how the tool loop fits into the broader tool calling architecture, see xref:api/tools.adoc#_the_tool_calling_loop[Tool Calling: The Tool Calling Loop].
...
Alternatively, enable schema validation directly on an `entity()` call without configuring the advisor manually, via xref:api/chatclient.adoc#_entityparamspec[`EntityParamSpec`]:
