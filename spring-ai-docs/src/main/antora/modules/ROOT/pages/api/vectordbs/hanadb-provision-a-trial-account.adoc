...
Let us start with creating a link:https://temp-mail.org/en/[temporary email] for registration purposes

image::../images/hanadb/0.png[width=800]

Note: do not close the above window, otherwise a new email id would get generated.
...
We receive a message to check our emails
...
It will prompt to create a `password`. Click `Submit`
...
We receive an OTP on the phone number. Key in the `code` and `continue`
...
Click `Create` to create a subscription

image::../images/hanadb/20.png[width=800]

TODO: add a slide to show the selections

Notice that `SAP Hana Cloud` subscription is now created. Click `Users` on the left panel

image::../images/hanadb/21.png[width=800]

Select the username (temporary email that we supplied earlier) and click `Assign Rolw Collection`
...
Notice that our `user` now has all the 3 role collections. Click `Instances and Subscriptions`
...
Select Type as `SAP HANA Cloud, SAP HANA Database`. Click `Next`
...
Notice that the provisioning of SAP Hana DB instance has started. It takes some time to provision.
...
Navigate to `hana_dev_db -> Catelog -> Tables` to find our table `CRICKET_WROLD_CUP`
...
Notice that the table data is now displayed. There are now rows as we didn't create any touples yet.
