...
This page is the reference for the builder API, configuration options, hook methods, and extension patterns. For the conceptual overview of the loop, see xref:api/tools.adoc#_the_tool_calling_loop[The Tool Calling Loop]. For the broader recursive-advisor pattern, see xref:api/advisors-recursive.adoc[Recursive Advisors].
...
The default `ToolCallingAdvisor.DEFAULT_ORDER` is `Ordered.HIGHEST_PRECEDENCE + 300`. This is higher than the default `MessageChatMemoryAdvisor` order (`HIGHEST_PRECEDENCE + 200`), which places memory advisors **outside** the tool loop by default. See xref:api/tools.adoc#_memory_and_the_tool_loop[Memory and the Tool Loop].
...
TIP: With the auto-registered `ToolCallingAdvisor`, `DefaultChatClient` detects any `MemoryAdvisor` placed inside the loop and disables internal history automatically — you don't need to call `disableInternalConversationHistory()` yourself. The manual call is only needed when constructing `ToolCallingAdvisor` directly. See xref:api/tools.adoc#_inside_the_loop[Memory: Inside the Loop].
...
To replace the default in a Spring Boot application, register your subclass via auto-configuration — see xref:api/tools.adoc#_auto_configuration_integration[Custom ToolAdvisor: Auto-Configuration Integration].

[[user-controlled-streaming]]
== User-Controlled Streaming

xref:api/tools.adoc#_user_controlled_tool_execution[User-controlled tool execution] covers the blocking variant; this section covers streaming.
...
NOTE: If the model requests multiple tool calls in a single iteration, `returnDirect` is only honored if **all** the called tools have `returnDirect = true`. Otherwise the results are sent back to the model and the loop continues. See xref:api/tools.adoc#_return_direct[Return Direct] for declaring the flag on tool definitions.
...
With auto-registration off, tools passed via `.tools(...)` are sent to the model but tool calls in the response are **not** executed automatically. You're then in xref:api/tools.adoc#_user_controlled_tool_execution[user-controlled mode].

[[see-also]]
== See Also

* xref:api/tools.adoc#_the_tool_calling_loop[Tool Calling: The Tool Calling Loop] — conceptual overview
* xref:api/tools.adoc#_memory_and_the_tool_loop[Tool Calling: Memory and the Tool Loop] — inside/outside ordering
* xref:api/tools.adoc#_extending_the_loop[Tool Calling: Extending the Loop] — auto-configuration extension point for custom subclasses
