...
| spring.ai.mistralai.chat.model | This is the Mistral AI Chat model to use | `open-mistral-7b`, `open-mixtral-8x7b`, `open-mixtral-8x22b`, `mistral-small-latest`, `mistral-large-latest`
| spring.ai.mistralai.chat.temperature | The sampling temperature to use that controls the apparent creativity of generated completions. Higher values will make output more random while lower values will make results more focused and deterministic. It is not recommended to modify `temperature` and `top_p` for the same completions request as the interaction of these two settings is difficult to predict. | 0.8
| spring.ai.mistralai.chat.max-tokens | The maximum number of tokens to generate in the chat completion. The total length of input tokens and generated tokens is limited by the model's context length. | -
| spring.ai.mistralai.chat.safe-prompt | Indicates whether to inject a security prompt before all conversations. | false
| spring.ai.mistralai.chat.random-seed | This feature is in Beta. If specified, our system will make a best effort to sample deterministically, such that repeated requests with the same seed and parameters should return the same result. | -
...
| spring.ai.mistralai.chat.tool-callbacks | Tool Callbacks to register with the ChatModel. | -
|====
...
TIP: All properties prefixed with `spring.ai.mistralai.chat` can be overridden at runtime by adding request-specific <<chat-options>> to the `Prompt` call.
...
`MistralAiChatModel` does not execute tool calls internally.
...
* **xref:api/tools.adoc#_user_controlled_tool_execution[User-controlled tool execution]** — use `DefaultToolCallingManager` directly when you need full control over the loop.
...
Set `internalToolExecutionEnabled(false)` on the options and drive the loop yourself using `DefaultToolCallingManager`.
...
You can configure the reasoning effort using the `reasoningEffort` property in `MistralAiChatOptions`.
This allows you to adjust the amount of computational effort the model spends on reasoning before generating a response.
