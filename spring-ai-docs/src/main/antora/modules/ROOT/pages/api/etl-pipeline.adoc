...
Spring AI provides two Tika implementations:

* *Embedded* (`TikaDocumentReader`): Uses Apache Tika directly in-process. Best for simple deployments.
* *Remote* (`TikaRemoteDocumentReader`): Uses a Tika Server instance over HTTP. Use when you need shared instances, additional language support, or want to reduce memory footprint. Run Tika Server via Docker: `docker run -p 9998:9998 apache/tika:latest`
...
Use `TikaRemoteDocumentReader` when running Tika as a separate server (e.g. via Docker):

[source,java]
----
// Default: http://localhost:9998
TikaRemoteDocumentReader reader = new TikaRemoteDocumentReader(resource);
List<Document> docs = reader.get();

// Custom Tika Server URL
TikaRemoteDocumentReader reader = new TikaRemoteDocumentReader("http://tika-server:9998", resource);
List<Document> docs = reader.get();
----
