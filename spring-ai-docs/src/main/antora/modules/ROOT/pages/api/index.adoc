...
The xref:api/chatclient.adoc[ChatClient API] offers a fluent API for communicating with an AI Model, idiomatic to Spring developers and similar to `WebClient` or `RestClient`.

=== Advisors API

The xref:api/advisors.adoc[Advisors API] encapsulates recurring Generative AI patterns, transforms data sent to and from Language Models (LLMs), and provides portability across various models and use cases (e.g., memory, tool-calling, RAG).

=== MCP (Model Context Protocol)

xref:api/mcp/mcp-overview.adoc[MCP (Model Context Protocol)] - Seamless integration for building AI applications that consume MCP servers or expose Spring-based services to the AI ecosystem.
