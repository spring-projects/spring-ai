...
| spring.ai.deepseek.chat.thinking.type | Controls the switch between thinking mode and non-thinking mode. When set to `enabled`, the thinking mode is used; when set to `disabled`, the non-thinking mode is used. | -
| spring.ai.deepseek.chat.reasoning-effort | Controls the reasoning effort level for the DeepSeek model. For normal requests, `high` is the default; for complex Agent-style requests (such as Claude Code, OpenCode), `max` is used automatically. | -
...
`DeepSeekChatModel` supports tool calling. The model can request tool execution but does not run tools itself — Spring AI handles execution.
...
DeepSeek models support a switch between thinking mode and non-thinking mode.
The thinking mode can be controlled through the `Thinking` option, which accepts `Thinking.ENABLED` (thinking mode) or `Thinking.DISABLED` (non-thinking mode).
When thinking is disabled, the model does not produce any reasoning content.
...
The thinking mode can also be toggled with the `enableThinking()` and `disableThinking()` builder convenience methods, or configured globally through the `spring.ai.deepseek.chat.thinking.type` property.

=== Reasoning Effort

DeepSeek models let you control the reasoning effort level through the `ReasoningEffort` option.
`ReasoningEffort.HIGH` is the default for normal requests, while `ReasoningEffort.MAX` is used automatically for complex Agent-style requests (such as Claude Code, OpenCode).
...
The reasoning effort can also be set with the `reasoningEffortHigh()` and `reasoningEffortMax()` builder convenience methods, or configured globally through the `spring.ai.deepseek.chat.reasoning-effort` property.
...
DeepSeek models support text and vision modalities.

=== Vision

DeepSeek models offer vision multimodal support through OpenAI-compatible `image_url` content parts.
Refer to the link:https://api-docs.deepseek.com/guides/vision[Vision] guide for more information.
An image can be provided either as a base64-encoded data URI or as a publicly accessible HTTP(S) URL.

Spring AI's link:https://github.com/spring-projects/spring-ai/blob/main/spring-ai-commons/src/main/java/org/springframework/ai/content/Media.java[Media] type carries the data and metadata of an image attachment.
When a `UserMessage` holds one or more `Media`, the image data is sent to the model as an `image_url` content part.

Below is an example sending a base64-encoded image from a local resource together with the user text:
...
ChatResponse response = chatModel.call(new Prompt(userMessage,
        DeepSeekChatOptions.builder()
            .model(DeepSeekApi.ChatModel.DEEPSEEK_V4_FLASH.getValue())
            .build()));
----
