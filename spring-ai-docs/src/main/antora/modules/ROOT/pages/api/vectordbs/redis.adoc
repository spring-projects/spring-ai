...
|`spring.ai.vectorstore.redis.dimensions` | The vector size of the index. Sizes an index created by `initialize-schema` and is the size `upsert` checks vectors against. If not set, it is read from the existing index, or taken from the `EmbeddingModel`. | -
...
CAUTION: A vector whose width does not match the index is written successfully and then never indexed, so the row exists but no search returns it.
The only trace is the `hash_indexing_failures` counter in `FT.INFO`.
