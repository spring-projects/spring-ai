...
The prefix `spring.ai.stabilityai` is used as the property prefix that lets you configure the HTTP client for Stability AI API calls.

[cols="3,5,1"]
|====
| Property | Description | Default

| spring.ai.stabilityai.connect-timeout | HTTP connection timeout | -
| spring.ai.stabilityai.read-timeout | HTTP read timeout | -
| spring.ai.stabilityai.redirects | HTTP redirect strategy (FOLLOW, NORMAL, DONT_FOLLOW) | -
| spring.ai.stabilityai.http.ssl.bundle | SSL bundle name for secure connections | -
|====
