...
Spring AI supports Cohere's text embedding models.
...
Spring AI provides Spring Boot auto-configuration for the Cohere Embedding Model.
...
| spring.ai.cohere.embedding.enabled (Removed and no longer valid) | Enable Cohere embedding model.  | true
...
If you are not using Spring Boot, you can manually configure the Cohere Embedding Model.
