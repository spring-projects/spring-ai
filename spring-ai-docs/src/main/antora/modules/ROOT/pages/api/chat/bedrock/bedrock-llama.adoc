...
| spring.ai.bedrock.llama.chat.options.max-gen-len          | Specify the maximum number of tokens to use in the generated response. The model truncates the response once the generated text exceeds maxGenLen. | AWS Bedrock default
|====

Look at https://github.com/spring-projects/spring-ai/blob/4ba9a3cd689b9fd3a3805f540debe398a079c6ef/models/spring-ai-bedrock/src/main/java/org/springframework/ai/bedrock/llama/BedrockLlamaChatModel.java[LlamaChatModel]  for other model IDs. The other value supported is `meta.llama2-13b-chat-v1`.
...
BedrockLlamaChatModel chatModel = new BedrockLlamaChatModel(converseApi,
    BedrockLlamaChatOptions.builder()
