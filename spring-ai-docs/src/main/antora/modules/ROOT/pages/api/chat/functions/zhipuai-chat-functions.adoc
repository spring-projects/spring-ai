...
Internally, Spring AI `ChatModel` will create an instance of a `ToolCallback` instance that adds the logic for it being invoked via the AI model.
...
Another way register a function is to create `ToolCallback` instance like this:
