...
[NOTE]
====
For the Gemini Developer API, you can share an API key with the Google GenAI Chat module by setting `spring.ai.google.genai.api-key`.
An embedding-specific key set through `spring.ai.google.genai.embedding.api-key` takes precedence over the shared key.
The shared key is used only when no embedding-specific API key or Vertex AI connection settings are configured.
For Vertex AI, configure the connection using the `spring.ai.google.genai.embedding` prefix; these settings do not fall back to the chat configuration.
====
...
| spring.ai.google.genai.embedding.api-key   | API key for Gemini Developer API. When provided, the client uses the Gemini Developer API instead of Vertex AI. Falls back to `spring.ai.google.genai.api-key` when no embedding-specific Vertex AI connection settings are configured. |  -
...
| spring.ai.google.genai.embedding.text.model | The https://ai.google.dev/gemini-api/docs/models/gemini#text-embedding[Google GenAI Text Embedding model] to use. Supported models include `text-embedding-004` and `text-multilingual-embedding-002` | text-embedding-004
| spring.ai.google.genai.embedding.text.task-type | The intended downstream application to help the model produce better quality embeddings. Available link:https://ai.google.dev/api/embeddings#tasktype[task-types]: `RETRIEVAL_QUERY`, `RETRIEVAL_DOCUMENT`, `SEMANTIC_SIMILARITY`, `CLASSIFICATION`, `CLUSTERING`, `QUESTION_ANSWERING`, `FACT_VERIFICATION`  | `RETRIEVAL_DOCUMENT`
| spring.ai.google.genai.embedding.text.title | Optional title, only valid with task_type=RETRIEVAL_DOCUMENT.  | -
| spring.ai.google.genai.embedding.text.dimensions | The number of dimensions the resulting output embeddings should have. Supported for model version 004 and later. You can use this parameter to reduce the embedding size, for example, for storage optimization.  | -
| spring.ai.google.genai.embedding.text.auto-truncate | When set to true, input text will be truncated. When set to false, an error is returned if the input text is longer than the maximum length supported by the model.  | true
|====
