...
float[] embedding = ...;
Document document = new Document("42", "some text", Map.of("source", "manual"));
...
Stores that implement it validate that every vector matches the index dimension before writing anything, so a wrong-sized vector fails fast and cannot partially write.
...
NOTE: A document carrying `CONTENT_REF` is a reference row regardless of what `isText()` reports. The empty text only satisfies the document's text-or-media rule; it is not content.
