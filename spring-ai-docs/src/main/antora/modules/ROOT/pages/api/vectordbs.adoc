...
While `add` computes the embeddings for you, `upsert` writes ones you supply yourself; see <<writing-precomputed-embeddings,Writing User-Supplied Embeddings>>.
...
NOTE: Azure Cosmos DB vector store support is available as an external module maintained by the Azure Cosmos DB team.
See https://azurecosmosdb.github.io/spring-ai/docs/index.html for more information.
...
`add` computes the embeddings for you.
If you already have the vectors, write them as they are with `upsert` instead; see <<writing-precomputed-embeddings,Writing User-Supplied Embeddings>>.

[[writing-precomputed-embeddings]]
=== Writing User-Supplied Embeddings
...
`EmbeddedDocument` rejects a vector that is empty or holds `NaN` or an infinity, which is worth knowing when the vectors come from a pipeline you do not control: a `NaN` would otherwise corrupt distance maths rather than fail.
...
The store works out that dimension from a `dimensions` value you configured, or otherwise reads it from the existing index, table, or collection, and asks the embedding model only as a last resort.
Elasticsearch is the one exception to the first step: its configured `dimensions` has a default, so it is trusted only when the store created the index itself.
So an application that only upserts into an existing index, or that configures `dimensions`, never contacts the embedding model and does not need to be able to reach it.
Where the dimension cannot be determined at all, the check is left to the database, which rejects the vector with its own error.
Redis cannot do that: it stores a wrong-sized vector without complaint and never finds it again, so `RedisVectorStore` refuses the write instead.
...
The query side:: `similaritySearch` with query text embeds the query with the store's own `EmbeddingModel`, so that model has to produce vectors in the same space and of the same width as the ones you upserted.
To keep the store's model out of it, embed the query yourself with the same model and search with the vector; see <<searching-with-precomputed-embeddings,Searching with a User-Supplied Query Embedding>>.
...
Document chunk = Document.builder()
    .text(chunkText)                                                  // real text, so add can embed it
    .metadata(DocumentMetadata.CONTENT_REF.value(), "articles/1042")  // where the full article lives
    .build();

this.vectorStore.add(List.of(chunk));
----

Only a row with empty text has to go through `upsert`, because `add` embeds the text and there would be nothing to embed.
...
WARNING: Two models are in play here and they have to produce vectors in the same space and of the same width: the one you embed the content with, and the store's own `EmbeddingModel`, which embeds the query inside `similaritySearch`.
Spring AI's `VertexAiMultimodalEmbeddingModel` puts text and images in one 1408-dimension space, but it implements `DocumentEmbeddingModel` rather than `EmbeddingModel`, so using it on both sides means wrapping it for the store.
Wire an ordinary text model instead and your queries land somewhere else entirely: an error if the widths differ, and quietly meaningless results if they happen to match.
...
[[searching-with-precomputed-embeddings]]
=== Searching with a User-Supplied Query Embedding

Searching with text embeds the query with the store's own `EmbeddingModel`.
If your application computes its embeddings itself, for example because the vectors were written with `upsert`, it can embed the query the same way and search with the vector:

[source,java]
----
float[] queryEmbedding = myEmbeddingPipeline.embed(question);

SearchRequest request = SearchRequest.builder()
    .topK(5)
    .similarityThreshold(0.7)
    .filterExpression("tenant == 'acme'")
    .build();

List<Document> documents = retriever.similaritySearch(queryEmbedding, request);
----

The request supplies everything except the query: `topK`, the similarity threshold, and the filter expression.
Its query text, if set, is ignored.
The store's `EmbeddingModel` is never used, so an application that writes with `upsert` and searches this way does not need to be able to reach it.

The query embedding has to come from the same model, with the same settings, as the vectors it is compared against.
A query vector from a different model is not rejected when its width happens to match; the search simply returns results that look plausible and mean nothing.
What is checked for you: the vector must not be empty or contain `NaN` or infinity, and its width must match the store's dimension when the store knows it.

Searching with a query embedding is opt-in per store, like `upsert`: the default implementation throws `UnsupportedOperationException`.
It is available on pgvector, Redis, Elasticsearch, and Qdrant.
