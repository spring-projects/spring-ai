...
Spring AI includes a batching feature to address a crucial limitation in the document embedding process.
Embedding models typically have an upper threshold for tokens and cannot process more than this threshold in a single embedding request.
When inserting a large number of documents into a vector store, the combined content may exceed this token threshold.
Batching solves this issue by breaking down the documents into smaller groups that fit within the model's limits.
This feature is implemented through the `BatchingStrategy` interface, which allows for processing documents in sub-batches rather than individually.
In addition to solving the threshold issue, batching can lead to significant performance benefits, especially when dealing with large numbers of documents or APIs with token limitations.
...
Several vector stores supported by Spring AI are already configured to use this default batching strategy.
...
It automatically batches documents when finding embeddings.
