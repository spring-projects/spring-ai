...
If you are going to run the database on a Mac ARM platform you need to use colima as the container runtime (link:https://github.com/abiosoft/colima).

----
docker run --rm --name oracle23ai -d -p 1521:1521 \
   -e ORACLE_PASSWORD=mlops \
   -e APP_USER=mlops \
   -e APP_USER_PASSWORD=mlops \
