...
Examples include OpenAI's GPT-4o, Google's Gemini, Anthropic's Claude3, and open source offerings Llama3.2, LLaVA and BakLLaVA are able to accept multiple inputs, including text images, audio and video and generate text responses by integrating these inputs.
...
* xref:api/chat/google-genai-chat.adoc#_multimodal[Google GenAI (e.g. gemini-2.0-flash, gemini-2.0-flash-lite models)]
