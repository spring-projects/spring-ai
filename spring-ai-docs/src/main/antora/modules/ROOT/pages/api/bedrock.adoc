...
[source,shell]
----
spring.ai.bedrock.aws.region=eu-north-1
...
6. Defaults to `US_EAST_1` if no region defined
...
NOTE: By default, all models are disabled. You have to enable the chosen Bedrock models explicitly using the `spring.ai.model.embedding=<model-name>` property.
...
| Model
| bedrock-cohere
| bedrock-titan (no batch support yet)
|====
...
* xref:api/embeddings/bedrock-cohere-embedding.adoc[Spring AI Bedrock Cohere Embeddings]: `spring.ai.model.embedding=bedrock-cohere`
* xref:api/embeddings/bedrock-titan-embedding.adoc[Spring AI Bedrock Titan Embeddings]: `spring.ai.model.embedding=bedrock-titan`
