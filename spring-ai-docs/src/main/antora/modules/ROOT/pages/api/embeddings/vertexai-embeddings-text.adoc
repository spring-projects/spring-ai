...
| spring.ai.vertex.ai.embedding.api-endpoint   | Vertex AI Embedding API endpoint. |  -
...
| spring.ai.vertex.ai.embedding.text.model | This is the link:https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported-models[Vertex Text Embedding model] to use | text-embedding-004
| spring.ai.vertex.ai.embedding.text.task-type | The intended downstream application to help the model produce better quality embeddings. Available link:https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/text-embeddings-api#request_body[task-types]  | `RETRIEVAL_DOCUMENT`
| spring.ai.vertex.ai.embedding.text.title | Optional title, only valid with task_type=RETRIEVAL_DOCUMENT.  | -
| spring.ai.vertex.ai.embedding.text.dimensions | The number of dimensions the resulting output embeddings should have. Supported for model version 004 and later. You can use this parameter to reduce the embedding size, for example, for storage optimization.  | -
| spring.ai.vertex.ai.embedding.text.auto-truncate | When set to true, input text will be truncated. When set to false, an error is returned if the input text is longer than the maximum length supported by the model.  | true
|====
