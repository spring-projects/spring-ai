...
The prefix `spring.ai.jlama.chat` is the property prefix for configuring Jlama chat.
...
| `spring.ai.jlama.chat.model`
| Model path. Supports a local file path or a HuggingFace model id in `owner/repo` format.
| -

| `spring.ai.jlama.chat.working-directory`
| Optional working directory for downloaded models.
| `${java.io.tmpdir}/spring-ai-jlama`

| `spring.ai.jlama.chat.options.temperature`
| Sampling temperature.
| `0.7`

| `spring.ai.jlama.chat.options.max-tokens`
| Maximum tokens to generate.
| `256`
|====

NOTE: If `spring.ai.jlama.chat.model` points to a HuggingFace id (`owner/repo`), Spring AI downloads the model automatically.
When `spring.ai.jlama.chat.working-directory` is not set, models are downloaded under `${java.io.tmpdir}/spring-ai-jlama`.

NOTE: Legacy properties under `spring.ai.jlama.*` are still accepted for backward compatibility.
...
spring.ai.jlama.chat.model=tjake/TinyLlama-1.1B-Chat-v1.0-Jlama-Q4
spring.ai.jlama.chat.working-directory=/tmp/jlama
spring.ai.jlama.chat.options.temperature=0.7
spring.ai.jlama.chat.options.max-tokens=256
----
