...
Messages are retrieved in ascending order (oldest-to-newest), which is the expected format for LLM conversation history. Ordering is maintained by a `sequence_id` column that records each message's position within its conversation.
...
NOTE: As of 2.0.0, the ordering column is `sequence_id BIGINT` (previously a `timestamp` column). This is a breaking schema change. If you are upgrading an existing application, see xref:upgrade-notes.adoc[] for migration steps.
