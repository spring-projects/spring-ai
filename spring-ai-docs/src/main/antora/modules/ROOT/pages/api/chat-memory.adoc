...
* `MessageChatMemoryAdvisor`. This advisor manages the conversation memory using the provided `ChatMemory` implementation. On each interaction, it retrieves the conversation history from the memory and includes it in the prompt as a collection of messages. This is the recommended advisor for managing conversation memory.
* `PromptChatMemoryAdvisor` _(deprecated since 1.1.3; use `MessageChatMemoryAdvisor` instead)_. This advisor retrieves the conversation history and appends it to the system prompt as XML-tagged, HTML-entity-escaped blocks. Custom system prompt templates are no longer supported.
* `VectorStoreChatMemoryAdvisor`. This advisor manages the conversation memory using the provided `VectorStore` implementation. On each interaction, it retrieves the conversation history from the vector store and appends it to the system message. Retrieved documents are wrapped in typed `<memory-entry>` XML elements with their content XML-escaped to reduce prompt-injection risk.
...
IMPORTANT: `PromptChatMemoryAdvisor` is deprecated since 1.1.3 and will be removed in a future release.
Migrate to `MessageChatMemoryAdvisor`, which passes conversation history as typed message objects rather than embedding it in the system prompt, and does not have the limitations described below.
...
`PromptChatMemoryAdvisor` embeds the conversation history into the system prompt.
Each message is wrapped in an XML tag with its role as the tag name, and the message content is HTML-entity-escaped:
...
Custom system prompt templates are not supported.
The `systemPromptTemplate()` builder method throws `UnsupportedOperationException` to prevent accidental use of templates that omit the built-in encoding.
If you previously relied on a custom template, migrate to `MessageChatMemoryAdvisor`.

=== VectorStoreChatMemoryAdvisor

==== Security Considerations

Retrieved document content is XML-escaped and wrapped in typed `<memory-entry>` XML elements before being interpolated into the system prompt:
...
The default system prompt template also instructs the model to treat the `LONG_TERM_MEMORY` section as historical data only, not as instructions.
This is a convention-level control that reduces, but does not eliminate, the risk of prompt injection from user-sourced content embedded in the system prompt.
For agent configurations with tool access, prefer architectures that keep user-sourced content in typed `Message` objects by using `MessageChatMemoryAdvisor` instead.
