  ...
  ai:
    openai:
      api:
        key: <api-key>
----
TIP: Check the list of xref:#opensearchvector-properties[configuration parameters] to learn about the default values and configuration options.
...
NOTE: Those (portable) filter expressions get automatically converted into the proprietary OpenSearch `WHERE` link:https://opensearch.org/docs/latest/opensearch/sql/[filter expressions].
