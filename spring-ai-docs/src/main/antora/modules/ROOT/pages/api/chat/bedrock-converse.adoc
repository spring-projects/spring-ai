...
| spring.ai.bedrock.aws.connection-timeout | Max duration to wait while establishing connection | 5s
| spring.ai.bedrock.aws.connection-acquisition-timeout | Max duration to wait for new connection from the pool | 30s
| spring.ai.bedrock.aws.async-read-timeout | Max duration spent reading asynchronous responses | 30s
...
`BedrockProxyChatModel` supports tool calling. The model can request tool execution but does not run tools itself — Spring AI handles execution.

For most applications, use `ChatClient` with the auto-registered `ToolCallingAdvisor` — see xref:api/tools.adoc[Tool Calling]. For low-level control where you drive the loop yourself (for example, when combining tool calling with prompt caching), see xref:api/tools/chatmodel-tool-calling.adoc[ChatModel Tool Calling].

=== Reasoning with Tool Calls

When extended thinking is enabled and the model decides to call a tool, the signed reasoning blocks from that response are automatically preserved and replayed in the follow-up request alongside the tool results.
Amazon Bedrock requires those reasoning blocks to be present in the replay — omitting them causes an API error.

No special configuration is needed.
Spring AI handles the replay transparently whether you use the auto-managed tool loop (`ChatClient` with `ToolCallingAdvisor`) or drive the loop manually via `ToolCallingManager`.
...
For more information, see the xref:api/structured-output.adoc[Structured Output] documentation.
...
The content blocks of a user turn are sent to the Converse API in the order of the xref:api/chatmodel.adoc#message-parts[message parts] of the `UserMessage`.
A message built with the `text(...)` and `media(...)` builder methods, or with the constructors, is sent with its text first, followed by its media, as shown in the examples above.

The link:https://docs.aws.amazon.com/nova/latest/userguide/prompting-vision-prompting.html[Amazon Nova prompting guidance] recommends placing media such as images or videos before the instructional text.
To control the order, add the parts directly and pass the message to the prompt:
...
UserMessage message = UserMessage.builder()
    .part(MediaPart.of(Media.builder()
        ...
        .data(new ClassPathResource("/test.video.mp4"))
        .build()))
    .part(TextPart.of("Explain what do you see in this video?"))
    .build();
...
The request then carries the video block before the text block.
Blank text parts are not sent, and when conversation history caching is enabled the cache point is still added after the last part.
