...
AWS Bedrock's https://docs.aws.amazon.com/bedrock/latest/userguide/prompt-caching.html[prompt caching feature] allows you to cache frequently used prompts to reduce costs and improve response times for repeated interactions. Bedrock supports explicit cache control — see xref:api/prompt-caching.adoc[Prompt Caching] for the core concepts, strategies, configuration properties, and portable API reference.

This section covers Bedrock-specific behavior and limitations.

=== Bedrock Cache Limits

* Maximum of **4 cache breakpoints** per request.
* Fixed **5-minute cache TTL** — the `promptCacheTtl` setting is ignored.
* Bedrock supports only `promptCacheStrategy`. Advanced options like `promptCacheMultiBlockSystemCaching`, per-message-type TTL, and `promptCacheContentLengthFunction` are not applicable.
...
=== Conversation History Caching with ChatClient
