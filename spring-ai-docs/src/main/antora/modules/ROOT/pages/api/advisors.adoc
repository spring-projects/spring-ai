...
It blocks the call to the model provider if the user input contains any of the configured sensitive words.
+
By default, the sensitive word matching is case-sensitive.
You can enable case-insensitive matching by setting the `ignoreCase` option to `true` via the builder.
+
[source,java]
----
// Case-sensitive matching (default)
SafeGuardAdvisor.builder()
    .sensitiveWords(List.of("kill", "attack"))
    .build();
