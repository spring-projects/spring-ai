...
This https://arxiv.org/pdf/2309.06275[Re-Reading Improves Reasoning in Large Language Models] article introduces a technique called Re-Reading (Re2) that improves the reasoning capabilities of large language models.
...
Then an advisor can be implemented to apply this technique to the user's input query.
...
public class ReReadingAdvisor implements CallAroundAdvisor, StreamAroundAdvisor {
	...
	private AdvisedRequest before(AdvisedRequest advisedRequest) {
		...
		advisedUserParams.put("re2_input_query", inputQuery);

        AdvisedRequest advisedRequest = AdvisedRequest.from(advisedRequest)
...
----
<1> You can control the order of execution by setting the order value. Lower values execute first.
