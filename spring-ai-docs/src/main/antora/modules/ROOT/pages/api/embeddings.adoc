...
The Embedding API in turn is used by higher-level components to implement Embedding Models for specific embedding models, such as OpenAI, Titan, Ollie, and others.
...
By default it returns `Document.getText()`, but embedding model implementations that support `MetadataMode` (such as OpenAI or Mistral AI) override this method to return `Document.getFormattedContent(MetadataMode)`, ensuring that document metadata is included in the text sent to the embedding API when configured.
...
* xref:api/embeddings/voyageai-embeddings.adoc[Spring AI Voyage AI Embeddings]
