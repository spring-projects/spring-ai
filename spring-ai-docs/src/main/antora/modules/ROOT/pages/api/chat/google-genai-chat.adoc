...
The prefix `spring.ai.google.genai` is used as the property prefix that lets you configure the HTTP client for Google GenAI API calls.

[cols="3,5,1", stripes=even]
|====
| Property | Description | Default

| spring.ai.google.genai.timeout | Timeout for the request | -
| spring.ai.google.genai.max-connections | Maximum number of connections allowed in the pool | -
| spring.ai.google.genai.max-connections-per-host | Maximum number of connections allowed per host | -
|====
