...
| spring.ai.google.genai.chat.options.include-thoughts | Enable thought signature propagation for multi-turn conversations. **Required** for Gemini 3 Pro with function calling to avoid validation errors. See <<thought-signatures>>. | false
...
Gemini 3 Pro introduces thought signatures, which enable the model to share its reasoning process during multi-turn conversations. When enabled, the model returns opaque thought signatures that must be passed back in subsequent turns to maintain conversational context.

=== Requirements

**IMPORTANT**: For Gemini 3 Pro with function calling, `includeThoughts` must be set to `true`. Failure to propagate thought signatures will result in 4xx validation errors from the API.

For Gemini 2.5 Pro and earlier models, thought signatures are optional but recommended for improved reasoning quality.
...
Spring AI automatically handles thought signature extraction and propagation in multi-turn conversations. You don't need to manually manage thought signatures - they are:

1. **Extracted** from model responses automatically
2. **Stored** in message metadata
3. **Propagated** in subsequent conversation turns

This ensures seamless function calling and multi-turn conversations without manual intervention.
...
NOTE: Enabling `includeThoughts` increases token usage as thought processes are included in responses. This impacts API costs but provides better reasoning transparency and improved context for multi-turn conversations.
