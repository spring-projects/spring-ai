...
| spring.ai.google.genai.chat.model | Supported https://ai.google.dev/gemini-api/docs/models[Google GenAI Chat models] to use, includes `gemini-2.5-flash`, `gemini-2.5-flash-lite`, `gemini-2.5-pro`, `gemini-3.1-pro-preview`, `gemini-3.1-flash-lite` and `gemini-3.5-flash`. | gemini-2.5-flash
...
| spring.ai.google.genai.chat.code-execution | Enable the Gemini code execution tool, allowing the model to generate and run Python code server-side and use the result in its response. The generated code and execution result are exposed in the response message metadata. Supported on both the Gemini Developer API and Vertex AI. See <<code-execution>>. | `true` or `false`, default `false`.
...
NOTE: `allowedFunctionNames` is only applied when mode is `ANY` or `VALIDATED`, and is silently ignored for `AUTO` and `NONE`.
...
Gemini provides a built-in https://ai.google.dev/gemini-api/docs/code-execution[code execution] tool. When enabled, the model can generate Python code, run it in a server-side sandbox, and use the result to produce its answer. This is useful for tasks that require precise computation, such as arithmetic, data processing, or algorithmic problems, where the model would otherwise be prone to errors.

Code execution is supported on both the Gemini Developer API and Vertex AI.

Enable it via application properties:

[source,application.properties]
----
spring.ai.google.genai.chat.code-execution=true
----
...
The generated code and its execution result are exposed in the response message metadata under the following keys:

[cols="1,3"]
|====
| Metadata Key | Description

| `executableCode` | The code generated by the model
| `executableCodeLanguage` | The language of the generated code (for example, `PYTHON`)
| `codeExecutionResult` | The output produced by running the code
| `codeExecutionOutcome` | The outcome of the execution (for example, `OUTCOME_OK`)
|====

[NOTE]
====
A code execution response is returned as multiple parts (the generated code, the execution result, and the final text), and each part maps to a separate `Generation` in the `ChatResponse`. The code execution metadata is therefore attached to the generation(s) representing the code and result parts, while the final answer text is carried by a separate generation. Iterate over `response.getResults()` to access all of them rather than assuming a single result.
====
...
for (Generation generation : response.getResults()) {
    ...
    Object generatedCode = metadata.get(GoogleGenAiChatModel.METADATA_EXECUTABLE_CODE);
    Object executionResult = metadata.get(GoogleGenAiChatModel.METADATA_CODE_EXECUTION_RESULT);
    // ...
}
----

NOTE: The Gemini API may not support combining the built-in code execution tool with client-side function (tool) calling in the same request. If you need both, consult the https://ai.google.dev/gemini-api/docs/code-execution[Gemini code execution documentation] for the model you are using.
...
| ✅ All levels
| ❌ Not supported
| Use `thinkingLevel` with any level including `MINIMAL` and `MEDIUM`.
