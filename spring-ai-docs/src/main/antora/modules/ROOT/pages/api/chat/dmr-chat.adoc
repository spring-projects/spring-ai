...
Spring AI integrates with the Docker Model Runner by reusing the existing xref::api/chat/openai-chat.adoc[OpenAI] client. 
For this you need to set the base-url to `http://localhost:12434/engines` and select one of the provided https://hub.docker.com/u/ai[LLM models].
...
for examples of using Docker Model Runner with Spring AI.
...
| spring.ai.openai.chat.base-url   | Optional overrides the spring.ai.openai.base-url to provide chat specific url. Must be set to `http://localhost:12434/engines` |  -
...
# The Docker Model Runner doesn't support embeddings, so we need to disable it.
