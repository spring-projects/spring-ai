...
| **Semantic**
| `spring-ai-tool-index-vectorstore`
| Natural-language queries, fuzzy matching

| **Keyword**
| `spring-ai-tool-index-lucene`
| Exact term matching, known tool names

| **Regex**
| `spring-ai-tool-index-regex`
...
The `ToolIndex` interface and its companion types (`ToolSearchRequest`, `ToolSearchResponse`, `ToolReference`) live in the `spring-ai-tool-search-tool-api` module under the package `org.springframework.ai.chat.client.advisor.tool.search.api`.
