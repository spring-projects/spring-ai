...
Spring AI supports three approaches to tool execution lifecycle management. The recommended approach is <<_advisor_controlled_tool_execution_with_toolcalladvisor,Advisor-Controlled Tool Execution>> via `ChatClient`. The framework also supports the legacy <<_framework_controlled_tool_execution,Framework-Controlled Tool Execution>> inside `ChatModel` (deprecated since 2.0.0) and a fully <<_user_controlled_tool_execution,User-Controlled Tool Execution>> mode. This section describes these three approaches.

=== Framework-Controlled Tool Execution

DEPRECATED: Internal tool execution inside `ChatModel` implementations is deprecated since 2.0.0 and will be removed in 3.0.0. Use <<_advisor_controlled_tool_execution_with_toolcalladvisor,Advisor-Controlled Tool Execution>> via `ChatClient` instead.
...
The logic determining whether a tool call is eligible for execution is handled by the `ToolExecutionEligibilityPredicate` interface.

DEPRECATED: `ToolExecutionEligibilityPredicate` is deprecated since 2.0.0 and will be removed in 3.0.0.

By default, the tool execution eligibility is determined by checking if the `internalToolExecutionEnabled` attribute of `ToolCallingChatOptions` is set to `true` (the default value), and if the `ChatResponse` contains any tool calls.
