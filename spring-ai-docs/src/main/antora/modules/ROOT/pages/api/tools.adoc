...
The same pattern applies to the streaming API, where each iteration's chunk `Flux` can be forwarded to a subscriber while being aggregated with `ChatClientMessageAggregator`. See xref:api/tools/tool-calling-advisor.adoc#_user_controlled_streaming[User-Controlled Streaming] for the full streaming variant.
