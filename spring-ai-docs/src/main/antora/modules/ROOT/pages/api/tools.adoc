...
When using `ChatClient`, tool call requests are handled automatically by the `ToolCallAdvisor`, which is registered in the advisor chain and manages the full tool execution lifecycle using the `ToolCallingManager` interface.

NOTE: `ChatModel` implementations previously handled tool execution internally. That approach is deprecated since 2.0.0 and will be removed in 3.0.0. Prefer using `ChatClient` with `ToolCallAdvisor` instead.
...
Under the hood, the `ChatClient` will generate a `ToolCallback` from each `@Tool`-annotated method in the tool class instance and pass them to the model. In case you prefer to generate the `ToolCallback`(s) yourself, you can use the `ToolCallbacks` utility class and pass them directly to `tools()`.
...
`tools()` also accepts `ToolCallback` and `ToolCallbackProvider` instances directly, alongside `@Tool`-annotated POJO instances and collections of any of these types.
...
NOTE: Calling `ChatModel` directly sends the tool definitions to the AI model, but tool calls in the response are *not* executed automatically. Use `ChatClient` (which auto-registers `ToolCallAdvisor`) for automatic execution, or implement xref:_user_controlled_tool_execution[user-controlled tool execution] to drive the loop yourself.
...
When using the programmatic specification approach, you can pass the `MethodToolCallback` instance directly to the `tools()` method on `ChatClient`.
...
When using the programmatic specification approach, you can add default tools to a `ChatClient.Builder` by passing the `MethodToolCallback` instance directly to `defaultTools()`.
If both default and runtime tools are provided, the runtime tools will completely override the default tools.
...
When using the programmatic specification approach, you can add default tools to a `ChatClient.Builder` by passing the `FunctionToolCallback` instance directly to `defaultTools()`.
If both default and runtime tools are provided, the runtime tools will completely override the default tools.
...
CAUTION: `toolNames()` is deprecated as of 2.0.0. Prefer injecting a resolved `ToolCallback` instance via `.tools(toolCallback)` instead.
...
CAUTION: `defaultToolNames()` is deprecated as of 2.0.0. Prefer injecting a resolved `ToolCallback` instance via `.defaultTools(toolCallback)` instead.
...
Spring AI supports three approaches to tool execution lifecycle management. The recommended approach is <<_framework_controlled_tool_execution,Framework-Controlled Tool Execution>> via `ChatClient`, which handles the tool calling loop automatically. For cases that require customizing the tool calling loop, <<_advisor_controlled_tool_execution_with_toolcalladvisor,Advisor-Controlled Tool Execution>> is available. When you need full manual control, a <<_user_controlled_tool_execution,User-Controlled Tool Execution>> mode is also available.

=== Framework-Controlled Tool Execution

When using `ChatClient`, Spring AI automatically handles the entire tool calling lifecycle through the `ToolCallAdvisor`, which is auto-registered in the advisor chain whenever tools are configured. All of this is done transparently — you provide the tools and ask your question; the framework manages the rest.

image::tools/framework-manager.jpg[Framework-controlled tool execution lifecycle, width=700, align="center"]

1. When we want to make a tool available to the model, we include its definition in the chat request and invoke the `ChatClient` API, which sends the request to the AI model.
2. When the model decides to call a tool, it sends a response with the tool name and input parameters.
3. The `ToolCallAdvisor` in the advisor chain intercepts the response and sends the tool call request to the `ToolCallingManager`.
4. The `ToolCallingManager` identifies and executes the tool with the provided input parameters.
5. The tool execution result is returned to the `ToolCallingManager`.
6. The `ToolCallingManager` returns the result to the `ToolCallAdvisor`.
7. The `ToolCallAdvisor` sends the tool execution result back to the AI model as a `ToolResponseMessage`.
8. The AI model generates the final response using the tool result as additional context and returns it to the caller via `ChatClient`.
...
To disable auto-registration globally for all calls from an auto-configured `ChatClient`, set the following property:
...
For cases that require customizing the tool calling loop, you can configure `ToolCallAdvisor` explicitly. `ToolCallAdvisor` implements the tool calling loop as part of the xref:api/chatclient.adoc#_advisors[advisor chain] and exposes several extension points:
...
* **Extensibility**: Custom `ToolExecutionEligibilityChecker` and hook methods allow fine-grained control over the loop

When the model requests a tool call, `ToolCallAdvisor` executes the tool via `ToolCallingManager` and sends the result back to the model, looping until no more tool calls are needed.
...
There are cases where you'd rather control the tool execution lifecycle yourself. To do so, invoke the `ChatModel` directly without adding `ToolCallAdvisor` to the advisor chain.
...
The `ToolCallbackResolver` is used internally by the `ToolCallingManager` to resolve tools dynamically at runtime, supporting both xref:_advisor_controlled_tool_execution_with_toolcalladvisor[] and xref:_user_controlled_tool_execution[].
...
The `ToolIndex` interface and its companion types (`ToolSearchRequest`, `ToolSearchResponse`, `ToolReference`) live in the `spring-ai-tool-search-tool` module under the package `org.springframework.ai.tool.toolsearch`.
The built-in index implementations (`LuceneToolIndex`, `VectorToolIndex`, `RegexToolIndex`) are also provided in this module.
