...
NOTE: Disabling internal history only changes what's forwarded to the rest of the chain each round — xref:#tool-call-limits[tool call limits] still see the complete history for the current turn regardless, so limits keep working correctly with a memory advisor nested inside the loop.
...
[WARNING]
====
A name conflict between two tool sources is only caught when the merged list goes through `ChatClient`.
`.tools(...)` and `.defaultTools(...)` validate the full merged list and throw `IllegalStateException` if any two callbacks share a name, so a conflict here fails the request instead of running the wrong tool.

That validation does not run if you call `ChatModel.call()`/`.stream()` directly, or build your own tool-calling loop, with a hand-assembled `ToolCallingChatOptions`.
On that path, tool execution resolves by name only: whichever callback comes first in the merged list wins, silently, with no error or log.
A remote MCP tool that happens to share a name with a trusted local tool can therefore execute in its place.

If you combine local and MCP tools outside `ChatClient`, either keep the sources namespaced so their names can't collide (see xref:api/mcp/mcp-client-boot-starter-docs.adoc#mcp-tool-name-prefix-generation[Tool Name Prefix Generation]), or call `ToolCallingChatOptions.validateToolCallbacks(...)` yourself on the merged list before use.
====
...
`DefaultToolCallingManager` caps how many times a tool can be called within a turn, to guard against runaway loops:
...
These defaults (`DefaultToolCallingManager.DEFAULT_MAX_CALLS_PER_TOOL` / `DEFAULT_MAX_TOTAL_TOOL_CALLS`) apply even without calling these methods. Use `unlimitedCallsPerTool()` / `unlimitedTotalToolCalls()` to disable a cap entirely.

==== Counting

Counts are scoped to the current turn only: only `ToolResponseMessage` entries from the last `UserMessage` onward are tallied, so history replayed by a `ChatMemory` advisor outside the loop never inflates the count for a new turn. Nothing is cached on the manager itself — counts are recomputed from `prompt.getInstructions()` on every call, so a single `ToolCallingManager` instance stays safe to share across concurrent requests. This also means disabling `ToolCallingAdvisor`'s internal conversation history (see xref:#inside-the-loop[Inside the Loop]) has no effect on limit counting — it always sees the complete turn history regardless of what's trimmed for the rest of the chain.
...
| `spring.ai.tools.limits.max-calls-per-tool.<name>` | Per-tool override; `-1` exempts that tool. | —
| `spring.ai.tools.limits.excluded-tools` | Tool names exempt from the per-tool limit. | —
| `spring.ai.tools.limits.max-total-tool-calls` | Max tool calls across all tools per turn. `-1` disables. | `150`
...
To enable the behavior — where a tool known to the resolver, but not attached to the request may be resolved by name and executed — enable the resolution fallback:
...
|====
| Property | Description | Default
| `spring.ai.tools.resolution.fallback.enabled` | If `true`, a requested tool that is absent from the request's tool callbacks may be resolved by name through the `ToolCallbackResolver` and executed. If `false`, only the tools attached to the request can be executed. | `false`
|====

When constructing a `ToolCallingManager` directly, the same switch is available on the builder via `.resolutionFallbackEnabled(true)` (default `false`).

WARNING: Enabling the resolution fallback makes every tool reachable through the resolver executable whenever the model names it, regardless of what a request attached — including risk-tier and destructive tools. Enable it only when you fully control which tools the resolver exposes.
