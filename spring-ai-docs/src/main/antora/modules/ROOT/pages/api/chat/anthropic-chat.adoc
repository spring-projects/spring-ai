...
Anthropic's https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching[Prompt Caching] reduces costs and latency by caching repeated context across API calls. Anthropic supports explicit cache control — see xref:api/prompt-caching.adoc[Prompt Caching] for the core concepts, strategies, configuration properties, and portable API reference.

This section covers Anthropic-specific behavior and limitations.

=== Anthropic Cache Limits

* Maximum of **4 cache breakpoints** per request. The implementation tracks breakpoint usage and stops adding cache control once the limit is reached.
* Supported TTL values: **5 minutes** (default) and **1 hour**. The portable `Duration`-based TTL is mapped to the nearest supported value.
* Anthropic supports all `PromptCacheChatOptions` fields including `promptCacheMultiBlockSystemCaching`, per-message-type TTL, per-message-type min content lengths, and custom `promptCacheContentLengthFunction`.

=== Multi-Block System Caching

When you have both a static system prompt and dynamic instructions, use `multiBlockSystemCaching` to cache only the static portion. Each system message becomes a separate cacheable block, with cache control applied to the second-to-last block:
