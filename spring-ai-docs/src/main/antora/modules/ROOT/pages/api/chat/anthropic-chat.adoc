...
| `spring.ai.anthropic.timeout` | Construction-time default timeout baked into the shared HTTP client at startup.
Can be overridden per call via `AnthropicChatOptions.timeout()` set on a `ChatClient` or per-request prompt options. | `60s`
...
When set via `ChatClient.Builder.defaultOptions()` or a per-request `ChatClient.prompt().options(...)`, it bounds only that call and overrides the `spring.ai.anthropic.timeout` construction-time default. | 60 seconds
...
The Anthropic API includes https://docs.anthropic.com/en/api/rate-limits#response-headers[rate limit headers] on every response that report your current request and token budgets along with when each window resets.
Spring AI exposes this data through the standard `ChatResponseMetadata#getRateLimit()` accessor, which returns an `AnthropicRateLimit` populated from the response headers.
...
Anthropic also reports separate buckets for input tokens and output tokens.
These are exposed by the `AnthropicRateLimit` concrete type:
...
Rate-limit metadata is populated on both the synchronous `call(Prompt)` and the streaming `stream(Prompt)` paths.
For streaming responses the headers are read once at stream start and attached to the response chunk that carries the final usage, so `getRateLimit()` resolves to a populated `AnthropicRateLimit` there.

NOTE: To read the response headers, the streaming path consumes the Anthropic SDK's blocking stream on a `Schedulers.boundedElastic()` worker, so a streaming call holds a worker thread for the duration of the stream.

== Tool Calling

`AnthropicChatModel` supports tool calling. Claude can request tool execution but does not run tools itself — Spring AI handles execution.

For most applications, use `ChatClient` with the auto-registered `ToolCallingAdvisor` — see xref:api/tools.adoc[Tool Calling]. For low-level control where you drive the loop yourself (for example, when combining tool calling with xref:_prompt_caching[prompt caching]), see xref:api/tools/chatmodel-tool-calling.adoc[ChatModel Tool Calling].
...
stream.subscribe(response -> {
    ...
    if (message.getMetadata().containsKey(AnthropicChatModel.THINKING_METADATA_KEY)) {
        // Thinking delta — getText() returns null so thinking does not mix into the
        // answer; the incremental thinking text is in the THINKING_TEXT_METADATA_KEY.
        System.out.print(message.getMetadata().get(AnthropicChatModel.THINKING_TEXT_METADATA_KEY));
    }
...
| **Thinking Delta (streaming)** | `THINKING_METADATA_KEY`, `THINKING_TEXT_METADATA_KEY` | Incremental thinking chunks during streaming. The `THINKING_METADATA_KEY` (`"thinking"`) is set to `true` and `THINKING_TEXT_METADATA_KEY` (`"thinkingText"`) holds the incremental thinking text. `getText()` returns null on these chunks, so thinking is not aggregated into the answer text; the full thinking text is also available via `AnthropicAssistantMessage.getThinkingContents()` on the aggregated response.
| **Text Block** | _(none)_ | The final answer text in `getText()`.
|====

=== Thinking with Tool Calls

When thinking is enabled and Claude decides to call a tool, the thinking blocks from
that response are automatically preserved and replayed in the follow-up request
alongside the tool results.
This is required by the Anthropic API — omitting the thinking blocks from the replay causes an API error.

No special configuration is needed.
Spring AI handles the replay transparently whether you use the auto-managed tool loop
(`ChatClient` with `ToolCallingAdvisor`) or drive the loop manually via `ToolCallingManager`.
...
include::partial$anthropic/anthropic-http-client-customization.adoc[]
