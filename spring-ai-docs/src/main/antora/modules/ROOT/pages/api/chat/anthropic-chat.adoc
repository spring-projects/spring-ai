...
To enable prompt caching, set the cache type on your user messages using either the constructor or builder pattern:

==== Using Constructor

[source,java]
----
// Create a message with ephemeral caching
UserMessage cachedMessage = new UserMessage("<the entire contents of Pride and Prejudice>", AnthropicCacheType.EPHEMERAL.name());
----
...
UserMessage message = UserMessage.builder()
    ...
    .cache(AnthropicCacheType.EPHEMERAL.name())
    .build();
----
...
System.out.println("Cache creation tokens: " + createdCacheToken.cacheCreationInputTokens());
System.out.println("Cache read tokens: " + createdCacheToken.cacheReadInputTokens());

// Second request with identical content - reads from cache
AnthropicApi.Usage readCacheToken = anthropicApi.chatCompletionEntity(chatCompletionRequest)
    .getBody()
    .usage();

System.out.println("Cache creation tokens: " + readCacheToken.cacheCreationInputTokens());
System.out.println("Cache read tokens: " + readCacheToken.cacheReadInputTokens());
----
...
5. **5-Minute Cache Lifetime**: Remember that ephemeral caches last 5 minutes and are refreshed each time the cached content is used.
...
The cache functionality extends the existing `AbstractMessage` class.
All message types that extend `AbstractMessage` now support the `getCache()` method to retrieve the cache configuration.

[source,java]
----
UserMessage message = new UserMessage("<the complete works of Tolkien>", AnthropicCacheType.EPHEMERAL.name());
String cacheType = message.getCache(); // Returns "EPHEMERAL"
----
