...
Spring AI surfaces this data through the standard `ChatResponseMetadata#getRateLimit()` accessor, which returns an `AnthropicRateLimit` populated from the response headers.
...
NOTE: Rate-limit metadata is only surfaced on the synchronous `call(Prompt)` path.
The streaming `stream(Prompt)` path uses Server-Sent Events, and the underlying transport does not expose the initial HTTP response headers to the streaming callback; `getRateLimit()` therefore resolves to `EmptyRateLimit` for streamed responses.
...
Prompt prompt = new Prompt(List.of(staticPrompt, dynamicContext, new UserMessage("...")), options);
----

This produces the following wire format:
