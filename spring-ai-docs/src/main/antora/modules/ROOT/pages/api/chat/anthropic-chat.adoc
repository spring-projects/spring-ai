...
NOTE: Rate-limit metadata is currently only populated on the synchronous `call(Prompt)` path.
The streaming `stream(Prompt)` path does not yet read rate-limit headers off the response; `getRateLimit()` returns an `EmptyRateLimit` for streamed responses.
