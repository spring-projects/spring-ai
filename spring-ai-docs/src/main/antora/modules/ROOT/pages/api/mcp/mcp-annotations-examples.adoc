    ...
    public ReadResourceResult getDocument(String id, McpMeta meta) {
        ...
        if (doc == null) {
            return ReadResourceResult.builder(List.of(
                TextResourceContents.builder("document://" + id,
                    "Document not found").mimeType("text/plain").build()
            )).build();
        }
        ...
        return ReadResourceResult.builder(List.of(
            TextResourceContents.builder("document://" + id,
                doc.getContent()).mimeType(doc.getMimeType()).build()
        )).build();
    }
    ...
    public Mono<ReadResourceResult> getAsyncData(String id) {
        return Mono.fromCallable(() -> loadDataAsync(id))
            ...
            .map(data -> ReadResourceResult.builder(List.of(
                TextResourceContents.builder("async-data://" + id,
                    data).mimeType("application/json").build()
            )).build());
    }
}
----
...
public class WeatherService {
    ...
    public String getTemperature2(McpSyncServerExchange exchange,
        ...
        if (exchange.getClientCapabilities().sampling() != null) {
            var samplingMessages = List.of(McpSchema.SamplingMessage.builder(McpSchema.Role.USER,
                    McpSchema.TextContent.builder(
                            "Please write a poem about this weather forecast (temperature is in Celsius). Use markdown format :\n "
                                    + new JsonHelper().toJson(weatherResponse)).build()).build());
