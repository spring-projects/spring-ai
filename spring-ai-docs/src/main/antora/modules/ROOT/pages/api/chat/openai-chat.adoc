...
`OpenAiChatModel` does not execute tool calls internally.
Tool execution must be handled externally using one of two supported approaches:

* **xref:api/tools.adoc#_advisor_controlled_tool_execution_with_toolcallingadvisor[ChatClient with ToolCallingAdvisor]** — the recommended approach for most use cases. `ToolCallingAdvisor` is automatically registered when tools are present and manages the tool-call loop transparently.
