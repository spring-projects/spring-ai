...
The prefix `spring.ai.openai` is used as the property prefix that lets you configure the HTTP client for OpenAI API calls.

[cols="3,5,1", stripes=even]
|====
| Property | Description | Default

| spring.ai.openai.connect-timeout | HTTP connection timeout | -
| spring.ai.openai.read-timeout | HTTP read timeout | -
| spring.ai.openai.redirects | HTTP redirect strategy (FOLLOW, NORMAL, DONT_FOLLOW) | -
| spring.ai.openai.http.ssl.bundle | SSL bundle name for secure connections | -
|====
