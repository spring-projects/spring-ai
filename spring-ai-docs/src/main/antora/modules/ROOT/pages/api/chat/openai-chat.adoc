...
OpenAI reports your current request and token budgets in https://developers.openai.com/api/docs/guides/rate-limits/#rate-limits-in-headers[rate limit response headers].
Spring AI exposes them through the standard `ChatResponseMetadata#getRateLimit()` accessor, which returns an `OpenAiRateLimit` populated from those headers.
...
If the response carries no rate-limit headers, `getRateLimit()` returns an `EmptyRateLimit` instance instead.

NOTE: Rate-limit metadata is populated on `call(Prompt)` only.
Reading it on `stream(Prompt)` would require the SDK's blocking streaming response, which would hold a worker thread for the whole stream, so `stream(Prompt)` reports an `EmptyRateLimit`.
...
To get reasoning summaries from official OpenAI models, use OpenAI's Responses API through xref:api/chat/openai-responses-chat.adoc[`OpenAiResponsesChatModel`].
That endpoint is also the only one that supports reasoning together with tool calling on GPT-5.4 and later.
