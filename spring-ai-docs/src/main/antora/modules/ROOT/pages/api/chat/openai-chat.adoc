...
Presently, only the `gpt-4-visual-preview` OpenAI model offers multimodal support.
For further details, refer to the link:https://platform.openai.com/docs/guides/vision[Vision] guide.
...
Below is a straightforward code example excerpted from link:https://github.com/spring-projects/spring-ai/blob/main/models/spring-ai-openai/src/test/java/org/springframework/ai/ollama/OpenAiChatClientIT.java[OllamaChatClientMultimodalIT.java], illustrating the fusion of user text with an image.
...
----
The picture shows a fruit bowl with a minimalistic design, featuring a wireframe structure. Inside the bowl,
there are two bananas and what appears to be a red apple. The bananas are resting on top of the apple, and
all fruits seem ripe and ready to eat. The bowl is placed on a flat surface, and the background is blurred,
which puts the focus on the fruit bowl. The lighting in the image highlights the yellow of the bananas and
the red of the apple, creating a warm, inviting composition.
----
