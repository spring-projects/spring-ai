...
If the collection is not created, the `QdrantVectorStore` will attempt to create one using the `Cosine` similarity and the configured `dimensions`, or the dimension of the configured `EmbeddingModel` if none is set.
...
|`spring.ai.vectorstore.qdrant.dimensions`| The vector size of the collection. Sizes a collection created by `initialize-schema` and is the size `upsert` checks vectors against. If not set, it is read from the existing collection, or taken from the `EmbeddingModel`. | -
|===
...
See xref:api/vectordbs.adoc#writing-precomputed-embeddings[Writing User-Supplied Embeddings] for details.
