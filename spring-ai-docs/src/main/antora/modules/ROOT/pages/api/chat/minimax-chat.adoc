...
| spring.ai.minimax.chat.max-tokens | The maximum number of tokens to generate in the chat completion. The total length of input tokens and generated tokens is limited by the model's context length. | -
...
| spring.ai.minimax.chat.presence-penalty | Number between -2.0 and 2.0. Positive values penalize new tokens based on whether they appear in the text so far, increasing the model's likelihood to talk about new topics. |  0.0f
| spring.ai.minimax.chat.frequency-penalty | Number between -2.0 and 2.0. Positive values penalize new tokens based on their existing frequency in the text so far, decreasing the model's likelihood to repeat the same line verbatim. | 0.0f
...
| spring.ai.minimax.chat.internal-tool-execution-enabled (Removed and no longer valid) | MiniMaxChatModel no longer executes tool calls internally. Use ChatClient with ToolCallAdvisor instead. | -
|====
...
`MiniMaxChatModel` does not execute tool calls internally.
...
public Function<Request, Response> weatherFunction() {
    return request -> new Response(String.format("%s°C", 10));
}
...
For scenarios where you need explicit control over the tool-call loop, you can drive the loop manually using `DefaultToolCallingManager`.
