...
TIP: The compatibility default remains `https://api.minimax.chat`. For the current hosted MiniMax platform and newer model families, override it to `https://api.minimax.io`. China users can override it with `https://api.minimaxi.com`.
...
| spring.ai.minimax.chat.options.model | This is the MiniMax Chat model to use | `abab6.5g-chat` (supported values include `MiniMax-M2.7`, `abab5.5-chat`, `abab5.5s-chat`, `abab6.5-chat`, `abab6.5g-chat`, `abab6.5t-chat`, and `abab6.5s-chat`)
...
| spring.ai.minimax.chat.options.reasoning-split | When enabled, MiniMax M2.7 returns interleaved reasoning in the `reasoning_details` field instead of mixing it into the visible response text. | -
...
== MiniMax M2.7 Interleaved Thinking

MiniMax M2.7 uses the same hosted `POST /v1/text/chatcompletion_v2` endpoint and the same OpenAI-style `tools` / `tool_choice` request format as the ABAB model family.

When `reasoningSplit` is enabled, MiniMax returns interleaved thinking in `reasoning_details`. Spring AI preserves those reasoning details in `AssistantMessage` metadata under the `reasoningDetails` key and automatically sends them back to MiniMax on follow-up tool-calling turns.

[source,java]
----
Prompt prompt = new Prompt("Check the weather in Madrid and explain your steps.",
    MiniMaxChatOptions.builder()
        .model(MiniMaxApi.ChatModel.MINIMAX_M2_7.getValue())
        .reasoningSplit(true)
        .build());

ChatResponse response = chatModel.call(prompt);

Object reasoningDetails = response.getResult().getOutput().getMetadata().get("reasoningDetails");
----

NOTE: Hosted MiniMax API responses expose tool calls as standard JSON `tool_calls`. Self-hosted MiniMax deployments that emit raw XML tool-call payloads are outside the scope of the Spring AI MiniMax integration.
