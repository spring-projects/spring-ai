...
This document is a quick reference for contributors to learn how to properly annotate the code base with JSpecify annotations.

For further reference, please consult the following:

* JSpecify https://jspecify.dev/docs/user-guide/[user-guide]
* Spring https://spring.io/blog/2025/03/10/null-safety-in-spring-apps-with-jspecify-and-null-away[Blog Post] about null safety
* https://github.com/uber/NullAway[NullAway] and https://errorprone.info/[ErrorProne]

== Solution
Spring AI uses JSpecify + NullAway + ErrorProne to enforce nullability checks.
...
----
From there on, any element in that package assumes non nullability by default (for its own members).
