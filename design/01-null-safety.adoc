...
Builder classes in Spring AI follow a consistent pattern for null safety:

* All optional fields in the builder are annotated with @Nullable,
* The uild() method validates required fields using Assert.notNull(),
* Collection and map fields are initialized to empty structures in the builder,
* The builder's clone() method performs deep copy of mutable collections.

Here is an example from AnthropicChatOptions:
...
* Required fields (like model) are enforced at build time,
* Optional fields can be safely ignored by callers,
* Mutable collections are properly isolated between instances,
* The builder can be safely reused or cloned.
